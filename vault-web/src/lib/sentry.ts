...
export const initSentry = (router?: Router<any, any, any, any, any>) => {
  ...
  const pickSampleRate = (pathname: string | null | undefined) => {
    if (!pathname) return defaultSampleRate
    if (
      highSampleRate !== undefined &&
      highSampleRoutes.some((route) => pathname.startsWith(route))
    ) {
      return highSampleRate
    }
    if (
      lowSampleRate !== undefined &&
      lowSampleRoutes.some((route) => pathname.startsWith(route))
    ) {
