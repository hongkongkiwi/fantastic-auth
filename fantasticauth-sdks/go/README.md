...
Official Go SDK for Vault authentication and user management.

## Installation

```bash
go get github.com/vault-dev/vaultauth
```
...
import (
    ...
    "github.com/vault-dev/vaultauth"
    "github.com/vault-dev/vaultauth/middleware"
)
...
func main() {
    ...
    // Apply middleware
    r.Use(vaultauth.MiddlewareGin(client, "/health", "/public"))
    
    // Protected route
    r.GET("/protected", func(c *gin.Context) {
        user, exists := vaultauth.GetCurrentUser(c)
        if !exists {
    ...
    r.GET("/admin", vaultauth.RequireAuth([]string{"admin", "owner"}, false), func(c *gin.Context) {
...
func main() {
    ...
    // Apply middleware
    e.Use(vaultauth.MiddlewareEcho(client, "/health"))
    
    // Protected route
    e.GET("/protected", func(c echo.Context) error {
        user, exists := vaultauth.GetCurrentUserEcho(c)
        if !exists {
    ...
    }, vaultauth.RequireAuthEcho([]string{"admin"}, false))
...
func main() {
    ...
    // Protected route
    protected := vaultauth.MiddlewareStdlib(client, "/health")(http.HandlerFunc(
        func(w http.ResponseWriter, r *http.Request) {
            user, exists := vaultauth.GetCurrentUserStdlib(r.Context())
            if !exists {
