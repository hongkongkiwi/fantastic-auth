...
async fn get_hosted_config(
    ...
    let config = build_hosted_config(&state, &query.tenant_id).await?;
...
async fn update_hosted_config(
    ...
    let config = build_hosted_config(&state, &tenant_id).await?;
...
async fn hosted_sign_in(
    ...
    let config = build_hosted_config(&state, &request.tenant_id).await?;
...
async fn hosted_oauth_start(
    ...
    let (oauth_config, _provider) = get_oauth_config(&state, &request.provider)?;
...
async fn hosted_oauth_callback(
    ...
    let stored = verify_hosted_oauth_state(&state, &request.state).await?;
    if stored.tenant_id != request.tenant_id {
        return Err(ApiError::BadRequest("OAuth tenant mismatch".to_string()));
    }

    let config = build_hosted_config(&state, &stored.tenant_id).await?;
    ...
    let (oauth_config, provider_enum) = get_oauth_config(&state, &stored.provider)?;
    let oauth_service = oauth_core::OAuthService::new(oauth_config);
