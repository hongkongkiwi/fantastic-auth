...
pub async fn list_users(
    ...
    let users: Vec<ScimUser> = rows
        .into_iter()
        .map(|row| row_to_scim_user(row, &auth_ctx.tenant_id))
        .collect();
...
pub async fn get_user(
    ...
    let user = row_to_scim_user(row, &auth_ctx.tenant_id);
    Ok(Json(user))
}
...
pub async fn update_user(
    ...
    let mut updated_user = row_to_scim_user(row, &auth_ctx.tenant_id);
...
pub async fn patch_user(
    ...
    let mut user = row_to_scim_user(row, &auth_ctx.tenant_id);
    let now = Utc::now();
    ...
    let updated_user = row_to_scim_user(row, &auth_ctx.tenant_id);
...
fn row_to_scim_user(row: ScimUserRow, _tenant_id: &str) -> ScimUser {
    let data = row.data;
    ...
    ScimUser {
        ...
        .ok(),
        groups: None, // TODO: Fetch from membership
...
fn apply_remove_by_path(
    ...
    if path == "emails" {
        ...
        // Remove specific email by value
        // TODO: Implement proper filter parsing for email removal
    } else {
        return Err((
            StatusCode::BAD_REQUEST,
...
async fn log_scim_audit(
    ...
    .bind(resource_type)
    .bind(resource_id)
    .bind(None::<String>) // TODO: Get from request
    .bind(None::<String>) // TODO: Get from request
    .bind(success)
    .bind(error)
    .execute(state.db.pool())
