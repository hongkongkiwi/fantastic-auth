# SAML 2.0 Implementation for Vault

## Overview

This module provides complete SAML 2.0 Service Provider (SP) support for Vault, enabling enterprise Single Sign-On (SSO) with identity providers like Okta, Azure AD, OneLogin, Auth0, and more.

## Features

### Core SAML Features
- **SP-initiated SSO** - Standard SAML flow initiated from Vault
...
- **Group/Role Mapping** - Map SAML groups to Vault roles
...
2. GET /saml/login
   ↓
3. Vault generates AuthnRequest (signed if configured)
   ↓
...
6. IdP POSTs SAML Response to /saml/acs
   ↓
7. Vault validates signature, extracts attributes
   ↓
...
1. User logs into IdP portal
   ↓
2. User clicks Vault application
   ↓
...
POST /api/v1/admin/sso/saml/connections
{
    ...
    "idp_sso_url": "https://your-domain.okta.com/app/vault/exk1234567890/sso/saml",
    "idp_slo_url": "https://your-domain.okta.com/app/vault/exk1234567890/slo/saml",
...
| SAML Attribute | Vault Field | Required |
|---------------|-------------|----------|
| `email` | `email` | Yes |
