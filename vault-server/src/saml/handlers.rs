...
async fn handle_logout_request(
    ...
    let idp_slo_url = saml_config.idp_slo_url
...
async fn create_session(
    ...
    // Use the existing auth service to create a session
    let (access_token, refresh_token, expires_in) = state.auth_service
        .create_session(tenant_id, &user.id, &user.email)
        .await
        .map_err(|_| ApiError::Internal)?;
