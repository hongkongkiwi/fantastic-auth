<script lang="ts">
   ...
   * SignUp Component
   * 
   * A complete sign-up form with email/password and OAuth support.
   * 
   * @example
   * ```svelte
   * <SignUp 
   ...
   * ```
   */
  import { useSignUp } from '../stores/auth.js';
  import type { SignUpProps, ApiError } from '../types.js';
...
<div class="vault-signup {className}">
  ...
  <form onsubmit={handleSubmit} class="vault-form">
    {#if requireName}
      <div class="vault-form-group">
        <label for="name" class="vault-label">Full name</label>
        <input
          id="name"
          type="text"
          bind:value={name}
    ...
    <div class="vault-form-group">
      ...
      <input
        ...
        class="vault-input"
      />
      <p class="vault-hint">Must be at least 8 characters</p>
    </div>
