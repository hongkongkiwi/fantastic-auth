...
pub fn routes() -> Router<AppState> {
    Router::new()
        // Get all settings
        .route("/settings/v2", get(get_all_settings))
        // Update entire settings (dangerous)
        .route("/settings/v2", patch(update_all_settings))
        ...
        .route(
            "/settings/v2/org",
            get(get_org_settings).patch(update_org_settings),
        )
        .route(
        ...
        .route(
            "/settings/v2/sms",
            get(get_sms_settings).patch(update_sms_settings),
        )
        .route(
            "/settings/v2/oauth",
        ...
        // Settings history
        .route("/settings/v2/history", get(get_settings_history))
        // Public settings (for hosted pages)
        .route("/settings/v2/public/:tenant_id", get(get_public_settings))
}
