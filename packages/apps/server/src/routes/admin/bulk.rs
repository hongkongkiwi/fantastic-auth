...
async fn start_import(
    ...
    if options.preview_mode {
        ...
        let processor = crate::bulk::import::ImportProcessor::new(
            app_state.db.clone(),
            storage_config.clone(),
        )
        .with_email_service(app_state.email_service.clone());

        // Process in background
        tokio::spawn(async move {
            job_clone.status = JobStatus::Processing;
            job_clone.started_at = Some(Utc::now());
            if let Err(e) = update_import_job(&app_state, &job_clone).await {
                error!(job_id = %job_id, error = %e, "Failed to mark import as processing");
            }

            if let Err(e) = processor.process(&mut job_clone).await {
                error!(job_id = %job_id, error = %e, "Import processing failed");
                job_clone.status = JobStatus::Failed;
                job_clone.error_message = Some(e.to_string());
                job_clone.completed_at = Some(Utc::now());
            }

            if let Err(e) = update_import_job(&app_state, &job_clone).await {
                error!(job_id = %job_id, error = %e, "Failed to persist import job status");
            }
        });
    } else {
...
async fn start_export(
    ...
    tokio::task::spawn_local(async move {
        ...
        job_clone.status = JobStatus::Processing;
        job_clone.started_at = Some(Utc::now());
        if let Err(e) = update_export_job(&app_state, &job_clone).await {
            error!(job_id = %job_id, error = %e, "Failed to mark export as processing");
        }
        ...
        if let Err(e) = update_export_job(&app_state, &job_clone).await {
            error!(job_id = %job_id, error = %e, "Failed to persist export job status");
        }
    });
...
/// Persist import job updates
async fn update_import_job(state: &AppState, job: &BulkImportJob) -> anyhow::Result<()> {
    sqlx::query(
        r#"UPDATE bulk_jobs
           SET status = $2,
               total_records = $3,
...
/// Persist export job updates
async fn update_export_job(state: &AppState, job: &BulkExportJob) -> anyhow::Result<()> {
    sqlx::query(
        r#"UPDATE bulk_jobs
           SET status = $2,
               total_records = $3,
