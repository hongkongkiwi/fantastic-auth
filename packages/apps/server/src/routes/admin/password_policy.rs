...
use crate::settings::models::{
    EnforcementMode as SettingsEnforcementMode, TenantPasswordPolicy,
};
...
impl From<TenantPasswordPolicy> for PasswordPolicyResponse {
    fn from(policy: TenantPasswordPolicy) -> Self {
        Self {
            min_length: policy.min_length,
            max_length: policy.max_length,
            require_uppercase: policy.require_uppercase,
            ...
            expiry_days: policy.expiry_days,
            check_breach: policy.check_breach,
            enforcement_mode: match policy.enforcement_mode {
                SettingsEnforcementMode::Block => "block".to_string(),
                SettingsEnforcementMode::Warn => "warn".to_string(),
                SettingsEnforcementMode::Audit => "audit".to_string(),
            },
            min_entropy: policy.min_entropy,
            prevent_user_info: policy.prevent_user_info,
        }
...
async fn get_password_policy(
    ...
    let settings = state
        .settings_service
        .get_settings(&current_user.tenant_id)
        .await?;

    Ok(Json(PasswordPolicyResponse::from(
...
async fn update_password_policy(
    ...
    let mut settings = state
        ...
        .get_settings(&current_user.tenant_id)
        .await?;
    let current = &settings.security.password_policy;
    ...
    let enforcement_mode = req
        .enforcement_mode
        .as_deref()
        .and_then(|m| match m {
            "block" => Some(SettingsEnforcementMode::Block),
            "warn" => Some(SettingsEnforcementMode::Warn),
            "audit" => Some(SettingsEnforcementMode::Audit),
            _ => None,
        })
        .unwrap_or(current.enforcement_mode);

    // Create merged policy and persist through settings service
    let merged = TenantPasswordPolicy {
    ...
    settings.security.password_policy = merged.clone();
    let updated_security = state
        .settings_service
        .update_security_settings(
            &current_user.tenant_id,
            settings.security,
            Some(&current_user.user_id),
            Some("password policy updated"),
        )
        .await?;

    Ok(Json(PasswordPolicyResponse::from(
        updated_security.password_policy,
    )))
}
...
async fn test_password_policy(
    ...
    let settings = state
        ...
        .get_settings(&current_user.tenant_id)
        .await?;
    let configured_policy = settings.security.password_policy.to_security_policy();
    let validator = crate::security::PasswordPolicyValidator::new(configured_policy);
    ...
    let result = validator
        .validate(
            &req.password,
            user_info.as_ref(),
            None::<&dyn crate::security::password_policy::PasswordHistoryChecker>,
            None,
        )
        .await;
