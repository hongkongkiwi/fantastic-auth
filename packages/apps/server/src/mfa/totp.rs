...
impl TotpMfaHandler {
    ...
    pub async fn setup(
        ...
        let backup_codes: Vec<String> = (0..10).map(|_| Self::generate_backup_code()).collect();

        // Store TOTP setup (created disabled; verify_setup enables it).
        state
            .db
            .mfa()
            .create_totp_method(tenant_id, user_id, &secret)
            .await
            .map_err(|e| {
                tracing::error!("Failed to store TOTP setup: {}", e);
                MfaError::Database(e)
            })?;
    ...
    pub async fn verify_setup(
        ...
        state
            .db
            .mfa()
            .verify_totp_method(tenant_id, user_id)
            .await
            .map_err(|e| {
    ...
    pub async fn verify_backup_code(
        ...
        state
            .db
            .mfa()
            .verify_backup_code(tenant_id, user_id, code)
            .await
            .map_err(|e| {
