...
function MfaContent() {
  ...
  const handleVerify = async () => {
    ...
    setError(
      'Hosted MFA challenge continuation is not available in this build. Return to sign-in and enter MFA code there.',
    )
  }
  ...
  const handleResend = async () => {
    ...
    try {
      setError(
        'Code resend is not available in this build. Return to sign-in to request a new challenge.',
      )
      setResendTimer(30)
      setCanResend(false)
    } catch (err) {
