...
import { AuthMfaRequiredError, useAuth } from '../hooks/useAuth'
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        await login(
          value.email,
          value.password,
          requiresMfaStep ? value.mfaCode || undefined : undefined,
          mfaToken,
        )
      } catch (error) {
            ...
            <Tabs value={loginMethod} onValueChange={(v) => setLoginMethod(v as LoginMethod)} className="mb-6">
              ...
              <TabsContent value="password" className="mt-4">
                ...
                >
                  ...
                  {requiresMfaStep && (
                    <p className="text-sm text-muted-foreground">
                      Multi-factor authentication is required. Enter your verification code to continue.
                    </p>
                  )}
                ...
                >
                  <SocialLoginButtons
                    onGoogleClick={
                      env.VITE_OAUTH_GOOGLE_ENABLED === 'true'
                        ? () => handleOAuthLogin('google')
                        : undefined
                    }
                    onGitHubClick={
                      env.VITE_OAUTH_GITHUB_ENABLED === 'true'
                        ? () => handleOAuthLogin('github')
                        : undefined
                    }
                    onMicrosoftClick={
                      env.VITE_OAUTH_MICROSOFT_ENABLED === 'true'
                        ? () => handleOAuthLogin('microsoft')
                        : undefined
                    }
                    onAppleClick={
                      env.VITE_OAUTH_APPLE_ENABLED === 'true'
