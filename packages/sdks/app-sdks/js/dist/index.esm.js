...
`;
    ...
    return jsx("style", { children: styleContent });
}
// ============================================================================
...
function ThemeProvider({ children, appearance = {}, defaultTheme, }) {
    ...
    return (jsxs(ThemeContext.Provider, { value: value, children: [jsx(GlobalStyles, { theme: theme, appendCss: appearance.appendCss }), children] }));
}
...
function VaultProvider({ children, config, initialUser, initialSessionToken, onAuthStateChange, appearance, }) {
    ...
    // Auth state
    const [authState, setAuthState] = useState(initialUser
    ...
    const signInWithMagicLink = useCallback(async (options) => {
        setLastError(null);
        await api.sendMagicLink(options);
    }, [api]);
    const signInWithOAuth = useCallback(async (options) => {
        setLastError(null);
    ...
    // ============================================================================
    const sendForgotPassword = useCallback(async (options) => {
        setLastError(null);
    ...
    const disableMfa = useCallback(async (method) => {
        try {
            setLastError(null);
    ...
    const setActiveOrganization = useCallback(async (orgId) => {
        try {
            setLastError(null);
    ...
    return (jsx(VaultContext.Provider, { value: value, children: jsx(ThemeProvider, { appearance: appearance, children: children }) }));
}
// ============================================================================
// Hook
// ============================================================================
...
function useSignUp() {
    ...
    const signUpWithOAuth = useCallback(async (options) => {
        setIsLoading(true);
        setError(null);
        try {
            await vault.signInWithOAuth(options);
...
function useMfaChallenge() {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const verify = useCallback(async (code, method) => {
        setIsLoading(true);
        setError(null);
        try {
...
function useOrganization() {
    ...
    const setActive = useCallback((orgId) => {
    ...
    const leave = useCallback(async (orgId) => {
        setIsLoading(true);
        setError(null);
        try {
     ...
     */
    const updateMemberRole = useCallback(async (orgId, userId, role) => {
        setIsLoading(true);
        setError(null);
        try {
...
function useCheckAuthorization() {
     ...
     */
    const check = useCallback((params) => {
...
function useBilling() {
     ...
     */
    const createPortalSession = useCallback(async (options) => {
        safeSetState(setIsLoading, true);
        safeSetState(setError, null);
        try {
     ...
     */
    const updateSubscription = useCallback(async (newPriceId) => {
        safeSetState(setIsLoading, true);
        safeSetState(setError, null);
        try {
            const response = await api.request('/api/v1/admin/billing/subscription', {
     ...
     */
    const reportUsage = useCallback(async (quantity, action = 'increment') => {
        try {
...
function useUsage() {
    ...
    const report = useCallback(async (quantity, action = 'increment') => {
        try {
            await api.request('/api/v1/admin/billing/usage', {
                method: 'POST',
                body: JSON.stringify({ quantity, action }),
...
// Component
// ============================================================================
const Button = forwardRef(({ children, variant = 'primary', size = 'md', isLoading = false, loadingText, fullWidth = false, elementClassName, disabled, className, style, ...props }, ref) => {
    ...
    return (jsx("button", { ref: ref, className: combinedClassName, style: buttonStyle, disabled: disabled || isLoading, ...props, children: isLoading ? (jsxs(Fragment, { children: [jsx("span", { className: "vault-spinner", style: {
                        width: '1rem',
                        height: '1rem',
...
// Component
// ============================================================================
const Input = forwardRef(({ label, error, helperText, id, disabled, required, inputClassName, labelClassName, errorClassName, fieldClassName, size = 'md', style, ...props }, ref) => {
    const { getElementClass, cssVariables } = useTheme();
    ...
    return (jsxs("div", { className: [getElementClass('formField'), fieldClassName]
            .filter(Boolean)
            .join(' '), children: [label && (jsxs("label", { htmlFor: inputId, className: [getElementClass('formFieldLabel'), labelClassName]
                    .filter(Boolean)
                    .join(' '), children: [label, required && (jsx("span", { style: { color: cssVariables['--vault-color-danger'], marginLeft: '0.25rem' }, children: "*" }))] })), jsx("input", { ref: ref, id: inputId, className: [getElementClass('formFieldInput'), inputClassName]
                    .filter(Boolean)
                    .join(' '), style: inputStyle, disabled: disabled, "aria-invalid": error ? 'true' : 'false', "aria-describedby": error ? `${inputId}-error` : undefined, ...props }), error && (jsx("span", { id: `${inputId}-error`, className: [getElementClass('formFieldError'), errorClassName]
                    .filter(Boolean)
                    .join(' '), role: "alert", children: error })), helperText && !error && (jsx("span", { style: {
                    display: 'block',
                    marginTop: '0.375rem',
                    fontSize: '0.75rem',
...
// Component
// ============================================================================
const Card = forwardRef(({ children, padding = 'md', width = 'md', centered = false, className, style, ...props }, ref) => {
    ...
    return (jsx("div", { ref: ref, className: [getElementClass('card'), 'vault-root', className]
            .filter(Boolean)
            .join(' '), style: cardStyle, ...props, children: children }));
});
Card.displayName = 'Card';
const CardHeader = forwardRef(({ title, subtitle, showLogo, logoUrl, className, style, children, ...props }, ref) => {
    ...
    return (jsxs("div", { ref: ref, className: [getElementClass('header'), className].filter(Boolean).join(' '), style: style, ...props, children: [shouldShowLogo && (jsx("div", { style: { marginBottom: '1rem' }, children: jsx("img", { src: layoutLogoUrl, alt: "Logo", style: { height: '40px', width: 'auto' } }) })), title && (jsx("h1", { className: getElementClass('headerTitle'), style: { margin: 0 }, children: title })), subtitle && (jsx("p", { className: getElementClass('headerSubtitle'), style: { margin: '0.5rem 0 0' }, children: subtitle })), children] }));
});
CardHeader.displayName = 'CardHeader';
const CardContent = forwardRef(({ children, spacing = 'md', className, style, ...props }, ref) => {
    const spacingStyles = {
        none: {},
...
const CardFooter = forwardRef(({ children, align = 'center', className, style, ...props }, ref) => {
    const alignStyles = {
        left: { textAlign: 'left' },
...
function Divider({ text, spacing = 'md', lineClassName, textClassName, }) {
    ...
    if (!text) {
        return (jsx("div", { className: getElementClass('dividerLine'), style: {
                height: '1px',
                backgroundColor: cssVariables['--vault-color-border'],
                ...spacingStyles[spacing],
            } }));
    }
    return (jsxs("div", { style: containerStyle, children: [jsx("div", { className: [getElementClass('dividerLine'), lineClassName]
                    .filter(Boolean)
                    .join(' ') }), jsx("span", { className: [getElementClass('dividerText'), textClassName]
                    .filter(Boolean)
                    .join(' '), children: text }), jsx("div", { className: [getElementClass('dividerLine'), lineClassName]
                    .filter(Boolean)
                    .join(' ') })] }));
}
...
function Header({ title, subtitle, showLogo, logoUrl, logo, align = 'center', titleClassName, subtitleClassName, children, }) {
    ...
    return (jsxs("header", { className: getElementClass('header'), style: {
            padding: '1.5rem 1.5rem 0.5rem',
            ...alignStyles[align],
        }, children: [shouldShowLogo && (jsx("div", { style: { marginBottom: '1rem' }, children: logo || (jsx("img", { src: layoutLogoUrl, alt: "Logo", style: { height: '40px', width: 'auto' } })) })), title && (jsx("h1", { className: [getElementClass('headerTitle'), titleClassName]
                    .filter(Boolean)
                    .join(' '), style: { margin: '0 0 0.5rem' }, children: title })), subtitle && (jsx("p", { className: [getElementClass('headerSubtitle'), subtitleClassName]
                    .filter(Boolean)
...
function SocialButton({ provider, variant = 'block', label, isLoading, elementClassName, disabled, style, children, ...props }) {
    ...
    return (jsxs("button", { type: "button", className: [baseClassName, elementClassName].filter(Boolean).join(' '), style: {
            ...(isIconVariant ? iconStyle : blockStyle),
            opacity: disabled || isLoading ? 0.6 : 1,
            cursor: disabled || isLoading ? 'not-allowed' : 'pointer',
            ...style,
        }, disabled: disabled || isLoading, ...props, children: [isLoading ? (jsx("span", { className: "vault-spinner", style: {
                    width: '1rem',
                    height: '1rem',
                } })) : (jsx(Icon, {})), !isIconVariant && jsx("span", { children: buttonLabel }), children] }));
}
function SocialButtons({ children, layout = 'vertical', className, }) {
    ...
    return (jsx("div", { className: [getElementClass('socialButtons'), className]
            .filter(Boolean)
...
// Icons
// ============================================================================
const ErrorIcon$1 = ({ color }) => (jsx("svg", { width: "20", height: "20", viewBox: "0 0 20 20", fill: "none", children: jsx("path", { fillRule: "evenodd", clipRule: "evenodd", d: "M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z", fill: color }) }));
const SuccessIcon$2 = ({ color }) => (jsx("svg", { width: "20", height: "20", viewBox: "0 0 20 20", fill: "none", children: jsx("path", { fillRule: "evenodd", clipRule: "evenodd", d: "M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z", fill: color }) }));
const WarningIcon = ({ color }) => (jsx("svg", { width: "20", height: "20", viewBox: "0 0 20 20", fill: "none", children: jsx("path", { fillRule: "evenodd", clipRule: "evenodd", d: "M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z", fill: color }) }));
const InfoIcon = ({ color }) => (jsx("svg", { width: "20", height: "20", viewBox: "0 0 20 20", fill: "none", children: jsx("path", { fillRule: "evenodd", clipRule: "evenodd", d: "M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z", fill: color }) }));
const icons = {
    error: ErrorIcon$1,
...
function Alert({ variant = 'info', title, showIcon = true, icon, onDismiss, children, className, style, ...props }) {
    ...
    return (jsxs("div", { className: [getElementClass('alert'), variantClasses[variant], className]
            .filter(Boolean)
            .join(' '), style: {
            display: 'flex',
            ...
            gap: '0.75rem',
            ...style,
        }, role: variant === 'error' ? 'alert' : 'status', ...props, children: [showIcon && (jsx("div", { style: { flexShrink: 0, marginTop: '0.125rem' }, children: icon || jsx(Icon, { color: color }) })), jsxs("div", { style: { flex: 1, minWidth: 0 }, children: [title && (jsx("div", { style: {
                            fontWeight: 600,
                            marginBottom: children ? '0.25rem' : 0,
                            color,
                        }, children: title })), children && (jsx("div", { style: { color }, children: children }))] }), onDismiss && (jsx("button", { type: "button", onClick: onDismiss, style: {
                    flexShrink: 0,
                    padding: '0.25rem',
                    margin: '-0.25rem',
                    background: 'none',
                ...
                }, "aria-label": "Dismiss", children: jsx("svg", { width: "16", height: "16", viewBox: "0 0 16 16", fill: "currentColor", children: jsx("path", { d: "M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z" }) }) }))] }));
}
...
function Spinner$3({ size = 'md', shimmer, shimmerWidth = '100%', shimmerHeight = '1rem', className, style, }) {
    ...
    if (useShimmer) {
        ...
        return (jsx("div", { className: ['vault-shimmer', className].filter(Boolean).join(' '), style: {
                width,
                height,
                borderRadius: cssVariables['--vault-border-radius'],
                ...style,
    ...
    // Spinner style
    return (jsx("div", { className: ['vault-spinner', className].filter(Boolean).join(' '), style: {
            width: sizeValue,
            height: sizeValue,
...
function SpinnerOverlay({ isLoading, children, size = 'lg', shimmer, opacity = 0.7, className, }) {
    ...
    return (jsxs("div", { className: className, style: {
            position: 'relative',
            display: 'inline-block',
        }, children: [children, isLoading && (jsx("div", { style: {
                    position: 'absolute',
                    inset: 0,
                    display: 'flex',
                    alignItems: 'center',
                ...
                }, children: jsx(Spinner$3, { size: size, shimmer: shimmer }) }))] }));
}
function Skeleton({ lines = 3, lineHeight = '1rem', gap = '0.5rem', className, }) {
    ...
    return (jsx("div", { className: className, style: {
            ...
            flexDirection: 'column',
            gap: gapValue,
        }, children: Array.from({ length: lines }).map((_, index) => (jsx(Spinner$3, { shimmer: true, shimmerWidth: index === lines - 1 ? '60%' : '100%', shimmerHeight: lineHeightValue }, index))) }));
}
...
function SignIn({ redirectUrl, onSignIn, onError, showMagicLink = true, showForgotPassword = true, oauthProviders = [], showWebAuthn = false, appearance, className, }) {
    ...
    const content = (jsx(SignInContent, { redirectUrl: redirectUrl, onSignIn: onSignIn, onError: onError, showMagicLink: showMagicLink, showForgotPassword: showForgotPassword, oauthProviders: oauthProviders, showWebAuthn: showWebAuthn, appearance: appearance, className: className }));
    // Wrap with ThemeProvider if not already themed
    if (!isThemed && appearance) {
        return (jsx(ThemeProvider, { appearance: appearance, children: content }));
    }
    return content;
}
// ============================================================================
...
function SignInContent({ redirectUrl, onSignIn, onError, showMagicLink, showForgotPassword, oauthProviders, showWebAuthn, className, }) {
    ...
    if (magicLinkSent) {
        return (jsxs(Card, { className: className, centered: true, children: [jsx(CardHeader, { title: "Check your email", subtitle: `We've sent a magic link to ${email}` }), jsxs(CardContent, { children: [jsx("p", { style: {
                                textAlign: 'center',
                                color: cssVariables['--vault-color-text-secondary'],
                                fontFamily: cssVariables['--vault-font-family'],
                            }, children: "Click the link in the email to sign in." }), jsx(Button, { variant: "ghost", onClick: () => {
                                setMagicLinkSent(false);
                                setEmail('');
                            }, children: "Back to sign in" })] })] }));
    }
    const SocialButtonsSection = oauthProviders && oauthProviders.length > 0 && (jsx(SocialButtons, { layout: socialButtonsVariant === 'iconButton' ? 'horizontal' : 'vertical', children: oauthProviders?.map((provider) => (jsx(SocialButton, { provider: provider, variant: socialButtonsVariant === 'iconButton' ? 'icon' : 'block', onClick: () => handleOAuth(provider), disabled: isLoading }, provider))) }));
    return (jsxs(Card, { className: className, centered: true, children: [jsx(CardHeader, { title: "Sign In" }), jsxs(CardContent, { children: [displayError && (jsx(Alert, { variant: "error", style: { marginBottom: '1rem' }, children: displayError })), socialButtonsPlacement === 'top' && SocialButtonsSection, socialButtonsPlacement === 'top' && (oauthProviders || []).length > 0 && (jsx(Divider, { text: "or" })), jsxs("form", { onSubmit: handleSubmit, children: [jsx(Input, { label: "Email", type: "email", value: email, onChange: (e) => setEmail(e.target.value), required: true, autoComplete: "email", disabled: isLoading, placeholder: "you@example.com" }), !useMagicLink && (jsxs("div", { style: { position: 'relative' }, children: [jsx(Input, { label: "Password", type: "password", value: password, onChange: (e) => setPassword(e.target.value), required: !useMagicLink, autoComplete: "current-password", disabled: isLoading }), showForgotPassword && (jsx("button", { type: "button", onClick: handleForgotPassword, style: {
                                            position: 'absolute',
                                            right: 0,
                                            top: 0,
                                            fontSize: '0.75rem',
                                            ...
                                            cursor: 'pointer',
                                            fontFamily: cssVariables['--vault-font-family'],
                                        }, children: "Forgot password?" }))] })), jsx(Button, { type: "submit", isLoading: isLoading, style: { marginTop: '0.5rem' }, children: useMagicLink ? 'Send Magic Link' : 'Sign In' })] }), showMagicLink && (jsx(Button, { variant: "ghost", onClick: () => {
                            setUseMagicLink(!useMagicLink);
                            resetError();
                            setLocalError(null);
                        }, children: useMagicLink ? 'Use password instead' : 'Use magic link instead' })), showWebAuthn && isWebAuthnSupported && (jsxs(Fragment, { children: [jsx(Divider, { text: "or" }), jsxs(Button, { variant: "secondary", onClick: handleWebAuthn, disabled: isLoading, children: [jsx("span", { style: { marginRight: '0.5rem' }, children: "\uD83D\uDD10" }), "Sign in with Passkey"] })] })), socialButtonsPlacement === 'bottom' && (oauthProviders || []).length > 0 && (jsx(Divider, { text: "or" })), socialButtonsPlacement === 'bottom' && SocialButtonsSection] })] }));
}
...
function SignUp({ redirectUrl, onSignUp, onError, oauthProviders = [], requireName = false, appearance, className, }) {
    ...
    const content = (jsx(SignUpContent, { redirectUrl: redirectUrl, onSignUp: onSignUp, onError: onError, oauthProviders: oauthProviders, requireName: requireName, appearance: appearance, className: className }));
    // Wrap with ThemeProvider if not already themed
    if (!isThemed && appearance) {
        return (jsx(ThemeProvider, { appearance: appearance, children: content }));
    }
...
function SignUpContent({ redirectUrl, onSignUp, onError, oauthProviders, requireName, className, }) {
    ...
    // Success state
    if (success) {
        return (jsxs(Card, { className: className, centered: true, children: [jsx(CardHeader, { title: "Account created!", subtitle: "Welcome! Your account has been successfully created." }), redirectUrl && (jsx(CardContent, { children: jsx("p", { style: {
                            textAlign: 'center',
                            color: cssVariables['--vault-color-text-secondary'],
                            fontFamily: cssVariables['--vault-font-family'],
    ...
    const SocialButtonsSection = oauthProviders && oauthProviders.length > 0 && (jsx(SocialButtons, { layout: socialButtonsVariant === 'iconButton' ? 'horizontal' : 'vertical', children: oauthProviders?.map((provider) => (jsx(SocialButton, { provider: provider, variant: socialButtonsVariant === 'iconButton' ? 'icon' : 'block', onClick: () => handleOAuth(provider), disabled: isLoading }, provider))) }));
    return (jsxs(Card, { className: className, centered: true, children: [jsx(CardHeader, { title: "Create Account" }), jsxs(CardContent, { children: [displayError && (jsx(Alert, { variant: "error", style: { marginBottom: '1rem' }, children: displayError })), socialButtonsPlacement === 'top' && SocialButtonsSection, socialButtonsPlacement === 'top' && (oauthProviders || []).length > 0 && (jsx(Divider, { text: "or" })), jsxs("form", { onSubmit: handleSubmit, children: [(requireName || name) && (jsx(Input, { label: "Full Name", type: "text", value: name, onChange: (e) => setName(e.target.value), required: requireName, autoComplete: "name", disabled: isLoading, placeholder: "John Doe" })), jsx(Input, { label: "Email", type: "email", value: email, onChange: (e) => setEmail(e.target.value), required: true, autoComplete: "email", disabled: isLoading, placeholder: "you@example.com" }), jsx(Input, { label: "Password", type: "password", value: password, onChange: (e) => setPassword(e.target.value), required: true, minLength: 12, autoComplete: "new-password", disabled: isLoading, placeholder: "Min 12 characters", helperText: "Must be at least 12 characters" }), jsx(Input, { label: "Confirm Password", type: "password", value: confirmPassword, onChange: (e) => setConfirmPassword(e.target.value), required: true, autoComplete: "new-password", disabled: isLoading, placeholder: "Re-enter your password" }), jsx(Button, { type: "submit", isLoading: isLoading, style: { marginTop: '0.5rem' }, children: "Create Account" })] }), socialButtonsPlacement === 'bottom' && (oauthProviders || []).length > 0 && (jsx(Divider, { text: "or" })), socialButtonsPlacement === 'bottom' && SocialButtonsSection] }), jsx(CardFooter, { children: jsxs("span", { style: {
                        fontSize: '0.875rem',
                        color: cssVariables['--vault-color-text-secondary'],
                        fontFamily: cssVariables['--vault-font-family'],
                    }, children: ["Already have an account?", ' ', jsx("a", { href: "/sign-in", style: {
                                color: cssVariables['--vault-color-primary'],
                                textDecoration: 'none',
                                fontWeight: 500,
                            }, children: "Sign in" })] }) })] }));
...
function UserButton({ showName = true, avatarUrl, onSignOut, menuItems = [], showManageAccount = true, appearance, className, }) {
    ...
    const content = (jsx(UserButtonContent, { showName: showName, avatarUrl: avatarUrl, onSignOut: onSignOut, menuItems: menuItems, showManageAccount: showManageAccount, appearance: appearance, className: className }));
    // Wrap with ThemeProvider if not already themed
    if (!isThemed && appearance) {
        return (jsx(ThemeProvider, { appearance: appearance, children: content }));
    }
...
function UserButtonContent({ showName, avatarUrl, onSignOut, menuItems, showManageAccount, className, }) {
    ...
    return (jsxs("div", { ref: menuRef, className: [getElementClass('userButton'), className].filter(Boolean).join(' '), style: { position: 'relative', display: 'inline-block' }, children: [jsxs("button", { type: "button", onClick: () => setIsOpen(!isOpen), className: getElementClass('userButtonTrigger'), "aria-expanded": isOpen, "aria-haspopup": "true", "aria-label": "User menu", children: [imageUrl ? (jsx("img", { src: imageUrl, alt: displayName, style: {
                            width: '2rem',
                            height: '2rem',
                            borderRadius: '50%',
                            objectFit: 'cover',
                        ...
                        } })) : (jsx("div", { className: getElementClass('avatarBox'), children: initial })), showName && (jsx("span", { style: {
                            fontWeight: 500,
                            fontFamily: cssVariables['--vault-font-family'],
                            color: cssVariables['--vault-color-text'],
                        }, children: displayName })), jsx(ChevronIcon$1, { isOpen: isOpen })] }), isOpen && (jsx("div", { className: getElementClass('userButtonPopover'), children: jsxs("div", { className: getElementClass('userButtonPopoverCard'), children: [jsxs("div", { style: {
                                padding: '0.75rem 1rem',
                                borderBottom: `1px solid ${cssVariables['--vault-color-border']}`,
                                        ...
                                        fontFamily: cssVariables['--vault-font-family'],
                                        color: cssVariables['--vault-color-text'],
                                    }, children: displayName }), jsx("div", { style: {
                                        fontSize: '0.75rem',
                                        fontFamily: cssVariables['--vault-font-family'],
                                        color: cssVariables['--vault-color-text-secondary'],
                                        marginTop: '0.125rem',
                                        wordBreak: 'break-all',
                                    }, children: user.email }), user.emailVerified === false && (jsx("span", { style: {
                                        display: 'inline-block',
                                        marginTop: '0.25rem',
                                    ...
                                    }, children: "Unverified" }))] }), jsxs("div", { className: getElementClass('menuList'), children: [showManageAccount && (jsx("button", { type: "button", onClick: handleManageAccount, className: getElementClass('menuItem'), role: "menuitem", children: "Manage account" })), menuItems?.map((item, index) => (jsx("button", { type: "button", onClick: () => {
                                        item.onClick();
                                        setIsOpen(false);
                                    }, className: getElementClass('menuItem'), role: "menuitem", children: item.label }, index))), (showManageAccount || (menuItems && menuItems.length > 0)) && (jsx("div", { style: {
                                        height: '1px',
                                        backgroundColor: cssVariables['--vault-color-border'],
                                        margin: '0.25rem 0',
                                    } })), jsx("button", { type: "button", onClick: handleSignOut, className: getElementClass('menuItem'), role: "menuitem", style: { color: cssVariables['--vault-color-danger'] }, children: "Sign out" })] })] }) }))] }));
}
// ============================================================================
// Chevron Icon
// ============================================================================
function ChevronIcon$1({ isOpen }) {
    const { cssVariables } = useTheme();
    return (jsx("svg", { width: "12", height: "12", viewBox: "0 0 12 12", fill: "none", style: {
            marginLeft: '0.25rem',
            transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)',
            transition: 'transform 0.2s ease',
            color: cssVariables['--vault-color-text-secondary'],
        }, children: jsx("path", { d: "M2.5 4.5L6 8L9.5 4.5", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round", strokeLinejoin: "round" }) }));
}
...
function UserProfile({ onUpdate, appearance, className, }) {
    ...
    if (!user) {
        return (jsx("div", { style: applyAppearance$8(styles$b.container, appearance), className: className, children: jsx("div", { style: styles$b.loading, children: "Loading profile..." }) }));
    }
    const displayError = localError || error?.message;
    return (jsxs("div", { style: applyAppearance$8(styles$b.container, appearance), className: className, children: [jsx("h1", { style: applyAppearance$8(styles$b.heading, appearance), children: "Profile" }), jsxs("div", { style: styles$b.tabs, children: [jsx(TabButton$1, { label: "Profile", isActive: activeTab === 'profile', onClick: () => setActiveTab('profile'), appearance: appearance }), jsx(TabButton$1, { label: "Security", isActive: activeTab === 'security', onClick: () => setActiveTab('security'), appearance: appearance }), jsx(TabButton$1, { label: "Danger Zone", isActive: activeTab === 'danger', onClick: () => setActiveTab('danger'), appearance: appearance, isDanger: true })] }), displayError && (jsx("div", { style: applyAppearance$8(styles$b.error, appearance), role: "alert", children: displayError })), successMessage && (jsx("div", { style: applyAppearance$8(styles$b.success, appearance), role: "status", children: successMessage })), activeTab === 'profile' && (jsxs("div", { style: styles$b.section, children: [jsxs("div", { style: styles$b.sectionHeader, children: [jsx("h2", { style: styles$b.sectionTitle, children: "Personal Information" }), !isEditing && (jsx("button", { onClick: () => setIsEditing(true), style: applyAppearance$8(styles$b.editButton, appearance), children: "Edit" }))] }), jsxs("div", { style: styles$b.fieldGroup, children: [jsxs("div", { style: styles$b.field, children: [jsx("label", { style: applyAppearance$8(styles$b.label, appearance), children: "Email" }), jsx("input", { type: "email", value: user.email, disabled: true, style: { ...applyAppearance$8(styles$b.input, appearance), backgroundColor: '#f3f4f6' } }), user.emailVerified ? (jsx("span", { style: styles$b.verifiedBadge, children: "\u2713 Verified" })) : (jsx("span", { style: styles$b.unverifiedBadge, children: "Unverified" }))] }), jsxs("div", { style: styles$b.field, children: [jsx("label", { style: applyAppearance$8(styles$b.label, appearance), children: "Full Name" }), jsx("input", { type: "text", value: formData.name, onChange: (e) => handleInputChange('name', e.target.value), disabled: !isEditing || isLoading, style: applyAppearance$8(styles$b.input, appearance), placeholder: "Your full name" })] }), jsxs("div", { style: styles$b.row, children: [jsxs("div", { style: { ...styles$b.field, flex: 1 }, children: [jsx("label", { style: applyAppearance$8(styles$b.label, appearance), children: "First Name" }), jsx("input", { type: "text", value: formData.givenName, onChange: (e) => handleInputChange('givenName', e.target.value), disabled: !isEditing || isLoading, style: applyAppearance$8(styles$b.input, appearance), placeholder: "First" })] }), jsxs("div", { style: { ...styles$b.field, flex: 1 }, children: [jsx("label", { style: applyAppearance$8(styles$b.label, appearance), children: "Last Name" }), jsx("input", { type: "text", value: formData.familyName, onChange: (e) => handleInputChange('familyName', e.target.value), disabled: !isEditing || isLoading, style: applyAppearance$8(styles$b.input, appearance), placeholder: "Last" })] })] }), jsxs("div", { style: styles$b.field, children: [jsx("label", { style: applyAppearance$8(styles$b.label, appearance), children: "Phone Number" }), jsx("input", { type: "tel", value: formData.phoneNumber, onChange: (e) => handleInputChange('phoneNumber', e.target.value), disabled: !isEditing || isLoading, style: applyAppearance$8(styles$b.input, appearance), placeholder: "+1 (555) 123-4567" })] })] }), isEditing && (jsxs("div", { style: styles$b.buttonGroup, children: [jsx("button", { onClick: handleSaveProfile, disabled: isLoading, style: applyAppearance$8(styles$b.primaryButton, appearance), children: isLoading ? 'Saving...' : 'Save Changes' }), jsx("button", { onClick: () => {
                                    setIsEditing(false);
                                    setFormData({
                                        name: user.profile?.name || '',
                                        givenName: user.profile?.givenName || '',
                                ...
                                }, disabled: isLoading, style: applyAppearance$8(styles$b.secondaryButton, appearance), children: "Cancel" })] }))] })), activeTab === 'security' && (jsxs("div", { style: styles$b.section, children: [jsx("h2", { style: styles$b.sectionTitle, children: "Change Password" }), jsxs("div", { style: styles$b.fieldGroup, children: [jsxs("div", { style: styles$b.field, children: [jsx("label", { style: applyAppearance$8(styles$b.label, appearance), children: "Current Password" }), jsx("input", { type: "password", value: passwordData.currentPassword, onChange: (e) => setPasswordData(prev => ({ ...prev, currentPassword: e.target.value })), style: applyAppearance$8(styles$b.input, appearance), placeholder: "Enter current password" })] }), jsxs("div", { style: styles$b.field, children: [jsx("label", { style: applyAppearance$8(styles$b.label, appearance), children: "New Password" }), jsx("input", { type: "password", value: passwordData.newPassword, onChange: (e) => setPasswordData(prev => ({ ...prev, newPassword: e.target.value })), style: applyAppearance$8(styles$b.input, appearance), placeholder: "Min 12 characters" })] }), jsxs("div", { style: styles$b.field, children: [jsx("label", { style: applyAppearance$8(styles$b.label, appearance), children: "Confirm New Password" }), jsx("input", { type: "password", value: passwordData.confirmPassword, onChange: (e) => setPasswordData(prev => ({ ...prev, confirmPassword: e.target.value })), style: applyAppearance$8(styles$b.input, appearance), placeholder: "Re-enter new password" })] })] }), jsx("button", { onClick: handleChangePassword, disabled: isLoading || !passwordData.currentPassword || !passwordData.newPassword, style: applyAppearance$8(styles$b.primaryButton, appearance), children: isLoading ? 'Changing...' : 'Change Password' }), jsxs("div", { style: styles$b.infoBox, children: [jsx("h3", { style: styles$b.infoTitle, children: "Account Information" }), jsxs("div", { style: styles$b.infoRow, children: [jsx("span", { style: styles$b.infoLabel, children: "Account ID:" }), jsx("code", { style: styles$b.code, children: user.id })] }), jsxs("div", { style: styles$b.infoRow, children: [jsx("span", { style: styles$b.infoLabel, children: "Created:" }), jsx("span", { children: new Date(user.createdAt).toLocaleDateString() })] }), jsxs("div", { style: styles$b.infoRow, children: [jsx("span", { style: styles$b.infoLabel, children: "Last Login:" }), jsx("span", { children: user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleDateString() : 'Never' })] })] })] })), activeTab === 'danger' && (jsxs("div", { style: styles$b.section, children: [jsx("h2", { style: { ...styles$b.sectionTitle, color: '#dc2626' }, children: "Danger Zone" }), jsxs("div", { style: styles$b.dangerBox, children: [jsx("h3", { style: styles$b.dangerTitle, children: "Delete Account" }), jsx("p", { style: styles$b.dangerText, children: "Once you delete your account, there is no going back. Please be certain." }), jsx("button", { onClick: handleDeleteAccount, disabled: isLoading, style: applyAppearance$8(styles$b.dangerButton, appearance), children: "Delete Account" })] })] }))] }));
}
...
function WebAuthnButton({ mode = 'signin', label, onSuccess, onError, appearance, className, }) {
    ...
    if (!isSupported) {
        return (jsx("div", { style: applyAppearance$7(styles$a.unsupported, appearance), className: className, children: jsx("span", { style: styles$a.unsupportedText, children: "Passkeys not supported on this device" }) }));
    }
    const displayError = localError || error?.message;
    ...
    if (showNameInput) {
        return (jsxs("div", { style: applyAppearance$7(styles$a.container, appearance), className: className, children: [jsxs("div", { style: styles$a.inputGroup, children: [jsx("input", { type: "text", value: passkeyName, onChange: (e) => setPasskeyName(e.target.value), placeholder: "Name your passkey (optional)", style: applyAppearance$7(styles$a.input, appearance), disabled: isLoading, autoFocus: true }), jsx("button", { onClick: handleClick, disabled: isLoading, style: applyAppearance$7(styles$a.button, appearance), children: isLoading ? 'Registering...' : 'Register' }), jsx("button", { onClick: handleCancel, disabled: isLoading, style: applyAppearance$7(styles$a.cancelButton, appearance), children: "Cancel" })] }), displayError && (jsx("div", { style: applyAppearance$7(styles$a.error, appearance), role: "alert", children: displayError }))] }));
    }
    return (jsxs("div", { style: applyAppearance$7(styles$a.container, appearance), className: className, children: [jsxs("button", { onClick: handleClick, disabled: isLoading, style: applyAppearance$7(styles$a.button, appearance), children: [jsx(PasskeyIcon, {}), jsx("span", { children: isLoading ? 'Please wait...' : buttonLabel })] }), displayError && (jsx("div", { style: applyAppearance$7(styles$a.error, appearance), role: "alert", children: displayError }))] }));
}
// Passkey Icon Component
function PasskeyIcon() {
    return (jsxs("svg", { width: "20", height: "20", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("circle", { cx: "12", cy: "8", r: "4" }), jsx("path", { d: "M12 12v8" }), jsx("path", { d: "M9 16h6" }), jsx("path", { d: "M8 20h8" }), jsx("rect", { x: "4", y: "2", width: "16", height: "20", rx: "2" })] }));
}
// Apply appearance variables
...
function VerifyEmail({ token, onVerified, onError, redirectUrl, appearance, className, }) {
    ...
    if (!token) {
        return (jsx("div", { style: applyAppearance$6(styles$9.container, appearance), className: className, children: jsxs("div", { style: styles$9.content, children: [jsx(EmailIcon, {}), jsx("h2", { style: applyAppearance$6(styles$9.title, appearance), children: "Verify your email" }), jsx("p", { style: styles$9.description, children: user?.emailVerified
                            ? 'Your email is already verified.'
                            : `We've sent a verification link to ${user?.email || 'your email address'}. Please check your inbox and click the link to verify.` }), !user?.emailVerified && (jsxs(Fragment, { children: [jsx("button", { onClick: handleResend, disabled: resendStatus === 'sending' || resendStatus === 'sent', style: applyAppearance$6(styles$9.button, appearance), children: resendStatus === 'sending'
                                    ? 'Sending...'
                                    : resendStatus === 'sent'
                                        ? 'Email sent!'
                                        : 'Resend verification email' }), error && (jsx("div", { style: applyAppearance$6(styles$9.error, appearance), role: "alert", children: error }))] }))] }) }));
    }
    // Show appropriate state based on verification status
    return (jsx("div", { style: applyAppearance$6(styles$9.container, appearance), className: className, children: jsxs("div", { style: styles$9.content, children: [status === 'verifying' && (jsxs(Fragment, { children: [jsx("div", { style: styles$9.spinner }), jsx("h2", { style: applyAppearance$6(styles$9.title, appearance), children: "Verifying your email..." })] })), status === 'success' && (jsxs(Fragment, { children: [jsx(SuccessIcon$1, {}), jsx("h2", { style: applyAppearance$6(styles$9.title, appearance), children: "Email verified!" }), jsxs("p", { style: styles$9.description, children: ["Your email has been successfully verified.", redirectUrl && ' Redirecting you...'] })] })), status === 'error' && (jsxs(Fragment, { children: [jsx(ErrorIcon, {}), jsx("h2", { style: applyAppearance$6(styles$9.title, appearance), children: "Verification failed" }), jsx("p", { style: styles$9.description, children: error || 'The verification link is invalid or has expired.' }), jsx("div", { style: styles$9.buttonGroup, children: jsx("button", { onClick: handleResend, disabled: resendStatus === 'sending', style: applyAppearance$6(styles$9.button, appearance), children: resendStatus === 'sending' ? 'Sending...' : 'Resend email' }) })] }))] }) }));
}
// Icon Components
function EmailIcon() {
    return (jsxs("svg", { width: "48", height: "48", viewBox: "0 0 24 24", fill: "none", stroke: "#0066cc", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", style: styles$9.icon, children: [jsx("rect", { x: "2", y: "4", width: "20", height: "16", rx: "2" }), jsx("path", { d: "m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" })] }));
}
function SuccessIcon$1() {
    return (jsxs("svg", { width: "48", height: "48", viewBox: "0 0 24 24", fill: "none", stroke: "#059669", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", style: styles$9.icon, children: [jsx("circle", { cx: "12", cy: "12", r: "10" }), jsx("path", { d: "m9 12 2 2 4-4" })] }));
}
function ErrorIcon() {
    return (jsxs("svg", { width: "48", height: "48", viewBox: "0 0 24 24", fill: "none", stroke: "#dc2626", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", style: styles$9.icon, children: [jsx("circle", { cx: "12", cy: "12", r: "10" }), jsx("line", { x1: "15", y1: "9", x2: "9", y2: "15" }), jsx("line", { x1: "9", y1: "9", x2: "15", y2: "15" })] }));
}
// Apply appearance variables
...
function ResetPassword({ token, onSuccess, onError, redirectUrl, appearance, className, }) {
    ...
    const content = (jsx(ResetPasswordContent, { token: token, onSuccess: onSuccess, onError: onError, redirectUrl: redirectUrl, appearance: appearance, className: className }));
    // Wrap with ThemeProvider if not already themed
    if (!isThemed && appearance) {
        return (jsx(ThemeProvider, { appearance: appearance, children: content }));
    }
...
function ResetPasswordContent({ token, onSuccess, onError, redirectUrl, className, }) {
    ...
    if (emailSent) {
        return (jsxs(Card, { className: className, centered: true, children: [jsx(CardHeader, { title: "Check your email", subtitle: `We've sent a password reset link to ${email}. Please check your inbox and follow the instructions.` }), jsxs(CardContent, { children: [jsx(SuccessIcon, {}), jsx(Button, { variant: "ghost", onClick: () => {
                                setEmailSent(false);
                                setEmail('');
    ...
    if (success) {
        return (jsxs(Card, { className: className, centered: true, children: [jsx(CardHeader, { title: "Password reset successful!", subtitle: `Your password has been successfully reset.${redirectUrl ? ' Redirecting you...' : ''}` }), jsx(CardContent, { children: jsx(SuccessIcon, {}) })] }));
    }
    // Request reset form
    if (mode === 'request') {
        return (jsxs(Card, { className: className, centered: true, children: [jsx(CardHeader, { title: "Reset your password", subtitle: "Enter your email address and we'll send you a link to reset your password." }), jsxs(CardContent, { children: [error && (jsx(Alert, { variant: "error", style: { marginBottom: '1rem' }, children: error })), jsxs("form", { onSubmit: handleRequestReset, children: [jsx(Input, { label: "Email", type: "email", value: email, onChange: (e) => setEmail(e.target.value), required: true, autoComplete: "email", disabled: isLoading, placeholder: "you@example.com" }), jsx(Button, { type: "submit", isLoading: isLoading, style: { marginTop: '0.5rem' }, children: "Send reset link" })] })] }), jsx(CardFooter, { children: jsx("a", { href: "/sign-in", style: {
                            fontSize: '0.875rem',
                            color: cssVariables['--vault-color-primary'],
                            textDecoration: 'none',
                            fontFamily: cssVariables['--vault-font-family'],
    ...
    // Reset password form
    return (jsxs(Card, { className: className, centered: true, children: [jsx(CardHeader, { title: "Create new password", subtitle: "Enter your new password below." }), jsxs(CardContent, { children: [error && (jsx(Alert, { variant: "error", style: { marginBottom: '1rem' }, children: error })), jsxs("form", { onSubmit: handleResetPassword, children: [jsx(Input, { label: "New Password", type: "password", value: password, onChange: (e) => setPassword(e.target.value), required: true, minLength: 12, autoComplete: "new-password", disabled: isLoading, placeholder: "Min 12 characters", helperText: "Must be at least 12 characters" }), jsx(Input, { label: "Confirm Password", type: "password", value: confirmPassword, onChange: (e) => setConfirmPassword(e.target.value), required: true, autoComplete: "new-password", disabled: isLoading, placeholder: "Re-enter your password" }), jsx(Button, { type: "submit", isLoading: isLoading, style: { marginTop: '0.5rem' }, children: "Reset password" })] })] })] }));
}
// Success Icon Component
function SuccessIcon() {
    const { cssVariables } = useTheme();
    return (jsx("div", { style: { textAlign: 'center', marginBottom: '1rem' }, children: jsxs("svg", { width: "48", height: "48", viewBox: "0 0 24 24", fill: "none", stroke: cssVariables['--vault-color-success'], strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("circle", { cx: "12", cy: "12", r: "10" }), jsx("path", { d: "m9 12 2 2 4-4" })] }) }));
}
...
function MFAForm({ challenge: propChallenge, onVerify, onError, allowBackupCode = true, appearance, className, }) {
    ...
    const content = (jsx(MFAFormContent, { challenge: propChallenge, onVerify: onVerify, onError: onError, allowBackupCode: allowBackupCode, appearance: appearance, className: className }));
    // Wrap with ThemeProvider if not already themed
    if (!isThemed && appearance) {
        return (jsx(ThemeProvider, { appearance: appearance, children: content }));
    }
...
function MFAFormContent({ challenge: propChallenge, onVerify, onError, allowBackupCode, className, }) {
    ...
    const [method] = useState(challenge?.method || 'totp');
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    ...
    return (jsxs(Card, { className: className, centered: true, width: "sm", children: [jsx(CardHeader, { title: "Two-factor authentication", subtitle: useBackupCode
                    ? 'Enter one of your backup codes to continue.'
                    : `Enter the 6-digit code from your ${getMethodLabel(method)}.` }), jsxs(CardContent, { children: [jsx(LockIcon, {}), error && (jsx(Alert, { variant: "error", style: { marginBottom: '1rem' }, children: error })), jsxs("form", { onSubmit: handleSubmit, children: [!useBackupCode ? (jsx("div", { style: {
                                    display: 'flex',
                                    justifyContent: 'center',
                                    gap: '0.5rem',
                                    marginBottom: '1.5rem',
                                }, children: code.map((digit, index) => (jsx("input", { ref: (el) => { inputRefs.current[index] = el; }, type: "text", inputMode: "numeric", maxLength: 1, value: digit, onChange: (e) => handleChange(index, e.target.value), onKeyDown: (e) => handleKeyDown(index, e), onPaste: handlePaste, disabled: isLoading, style: {
                                        width: '3rem',
                                        height: '3.5rem',
                                        fontSize: '1.5rem',
                                        fontWeight: 600,
                                        ...
                                        outline: 'none',
                                        transition: 'all 0.15s ease-in-out',
                                    }, "aria-label": `Digit ${index + 1}` }, index))) })) : (jsx("div", { style: { marginBottom: '1.5rem' }, children: jsx("input", { type: "text", value: code.join(''), onChange: (e) => {
                                    ...
                                    } }) })), jsx(Button, { type: "submit", isLoading: isLoading, disabled: code.join('').length !== 6, children: "Verify" })] }), allowBackupCode && (jsx(Button, { variant: "ghost", onClick: handleBackupCodeToggle, style: { marginTop: '0.5rem' }, children: useBackupCode
                            ? 'Use authenticator code instead'
                            : 'Use backup code instead' }))] }), jsx("div", { style: {
                    ...
                    borderTop: `1px solid ${cssVariables['--vault-color-border']}`,
                        ...
                        color: cssVariables['--vault-color-text-secondary'],
                        margin: 0,
                    }, children: ["Having trouble?", ' ', jsx("a", { href: "/support", style: {
                                color: cssVariables['--vault-color-primary'],
                                textDecoration: 'none',
...
function LockIcon() {
    const { cssVariables } = useTheme();
    return (jsx("div", { style: { textAlign: 'center', marginBottom: '1rem' }, children: jsxs("svg", { width: "48", height: "48", viewBox: "0 0 24 24", fill: "none", stroke: cssVariables['--vault-color-primary'], strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("rect", { x: "5", y: "11", width: "14", height: "10", rx: "2" }), jsx("circle", { cx: "12", cy: "16", r: "1" }), jsx("path", { d: "M8 11V7a4 4 0 0 1 8 0v4" })] }) }));
}
...
function OrganizationSwitcher({ hidePersonal = false, onSwitch, appearance, className, showSearch = true, pageSize = DEFAULT_PAGE_SIZE, }) {
    ...
    if (!isLoaded) {
        return (jsx("div", { style: applyAppearance$5(styles$8.skeleton, appearance), className: className, children: jsx("div", { style: styles$8.skeletonInner }) }));
    }
    const currentOrgName = activeOrg?.name || user?.profile?.name || user?.email || 'Personal';
    const hasOrganizations = organizations.length > 0;
    return (jsxs("div", { ref: menuRef, style: styles$8.container, className: className, children: [jsxs("button", { onClick: () => setIsOpen(!isOpen), style: applyAppearance$5(styles$8.button, appearance), "aria-expanded": isOpen, "aria-haspopup": "true", "aria-label": "Switch organization", children: [jsx(BuildingIcon$1, {}), jsx("span", { style: styles$8.buttonText, children: currentOrgName }), jsx(ChevronIcon, { isOpen: isOpen })] }), isOpen && (jsxs("div", { style: applyAppearance$5(styles$8.menu, appearance), role: "menu", children: [jsx("div", { style: styles$8.menuHeader, children: jsx("span", { style: styles$8.menuTitle, children: "Switch organization" }) }), showSearch && hasOrganizations && (jsxs("div", { style: styles$8.searchContainer, children: [jsx(SearchIcon, {}), jsx("input", { ref: searchInputRef, type: "text", value: searchQuery, onChange: (e) => {
                                    setSearchQuery(e.target.value);
                                    setCurrentPage(0);
                                }, placeholder: "Search organizations...", style: applyAppearance$5(styles$8.searchInput, appearance) }), searchQuery && (jsx("button", { onClick: () => {
                                    setSearchQuery('');
                                    searchInputRef.current?.focus();
                                }, style: styles$8.clearSearch, "aria-label": "Clear search", children: jsx(CloseIcon$1, {}) }))] })), searchQuery && (jsx("div", { style: styles$8.searchResults, children: filteredOrganizations.length === 0 ? (jsx("span", { children: "No organizations found" })) : (jsxs("span", { children: [filteredOrganizations.length, " result", filteredOrganizations.length !== 1 ? 's' : ''] })) })), jsxs("div", { style: styles$8.menuItems, children: [!hidePersonal && !searchQuery && currentPage === 0 && (jsxs("button", { onClick: () => handleSelectOrg(null), style: {
                                    ...applyAppearance$5(styles$8.menuItem, appearance),
                                    ...(activeOrg === null && styles$8.menuItemActive),
                                }, role: "menuitem", children: [jsx(UserIcon$2, {}), jsx("span", { style: styles$8.menuItemText, children: user?.profile?.name || user?.email || 'Personal Account' }), activeOrg === null && jsx(CheckIcon$1, {})] })), paginatedOrganizations.map((org) => (jsxs("button", { onClick: () => handleSelectOrg(org.id), style: {
                                    ...applyAppearance$5(styles$8.menuItem, appearance),
                                    ...(activeOrg?.id === org.id && styles$8.menuItemActive),
                                }, role: "menuitem", children: [jsx(BuildingIcon$1, { small: true }), jsxs("div", { style: styles$8.menuItemContent, children: [jsx("span", { style: styles$8.menuItemName, children: org.name }), jsx("span", { style: styles$8.menuItemSlug, children: org.slug })] }), activeOrg?.id === org.id && jsx(CheckIcon$1, {})] }, org.id))), !hasOrganizations && !isCreating && (jsxs("div", { style: styles$8.emptyState, children: [jsx("div", { style: styles$8.emptyIcon, children: jsx(BuildingIcon$1, {}) }), jsx("p", { style: styles$8.emptyTitle, children: "No organizations" }), jsx("p", { style: styles$8.emptyText, children: "Create one to collaborate with your team" })] })), searchQuery && filteredOrganizations.length === 0 && (jsxs("div", { style: styles$8.emptyState, children: [jsx("div", { style: styles$8.emptyIcon, children: jsx(SearchIcon, { large: true }) }), jsx("p", { style: styles$8.emptyTitle, children: "No results" }), jsx("p", { style: styles$8.emptyText, children: "Try a different search term" })] }))] }), filteredOrganizations.length > pageSize && (jsxs("div", { style: styles$8.pagination, children: [jsx("button", { onClick: () => setCurrentPage(p => Math.max(0, p - 1)), disabled: !hasPreviousPages, style: {
                                    ...styles$8.paginationButton,
                                    ...(!hasPreviousPages && styles$8.paginationButtonDisabled),
                                }, "aria-label": "Previous page", children: jsx(ChevronLeftIcon, {}) }), jsxs("span", { style: styles$8.paginationInfo, children: ["Page ", currentPage + 1, " of ", totalPages] }), jsx("button", { onClick: () => setCurrentPage(p => Math.min(totalPages - 1, p + 1)), disabled: !hasMorePages, style: {
                                    ...styles$8.paginationButton,
                                    ...(!hasMorePages && styles$8.paginationButtonDisabled),
                                }, "aria-label": "Next page", children: jsx(ChevronRightIcon, {}) })] })), jsx("div", { style: styles$8.divider }), isCreating ? (jsxs("form", { onSubmit: handleCreateOrg, style: styles$8.createForm, children: [error && (jsx("div", { style: applyAppearance$5(styles$8.createError, appearance), role: "alert", children: error })), jsx("input", { type: "text", value: newOrgName, onChange: (e) => setNewOrgName(e.target.value), placeholder: "Organization name", autoFocus: true, style: applyAppearance$5(styles$8.createInput, appearance), disabled: isLoading }), jsxs("div", { style: styles$8.createButtons, children: [jsx("button", { type: "submit", disabled: isLoading || !newOrgName.trim(), style: applyAppearance$5(styles$8.createButton, appearance), children: isLoading ? 'Creating...' : 'Create' }), jsx("button", { type: "button", onClick: () => {
                                            setIsCreating(false);
                                            setNewOrgName('');
                                            setError(null);
                                        }, style: applyAppearance$5(styles$8.cancelButton, appearance), children: "Cancel" })] })] })) : (jsxs("button", { onClick: () => setIsCreating(true), style: applyAppearance$5(styles$8.createOrgButton, appearance), children: [jsx(PlusIcon$1, {}), jsx("span", { children: "Create organization" })] }))] }))] }));
}
// Icon Components
function BuildingIcon$1({ small }) {
    return (jsxs("svg", { width: small ? 16 : 20, height: small ? 16 : 20, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("path", { d: "M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18Z" }), jsx("path", { d: "M6 12H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2" }), jsx("path", { d: "M18 9h2a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-2" }), jsx("path", { d: "M10 6h4" }), jsx("path", { d: "M10 10h4" }), jsx("path", { d: "M10 14h4" }), jsx("path", { d: "M10 18h4" })] }));
}
function UserIcon$2() {
    return (jsxs("svg", { width: 16, height: 16, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("path", { d: "M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" }), jsx("circle", { cx: "12", cy: "7", r: "4" })] }));
}
function PlusIcon$1() {
    return (jsxs("svg", { width: 16, height: 16, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("path", { d: "M5 12h14" }), jsx("path", { d: "M12 5v14" })] }));
}
function CheckIcon$1() {
    return (jsx("svg", { width: 16, height: 16, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: jsx("path", { d: "M20 6 9 17l-5-5" }) }));
}
function ChevronIcon({ isOpen }) {
    return (jsx("svg", { width: "12", height: "12", viewBox: "0 0 12 12", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", style: {
            transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)',
            transition: 'transform 0.2s ease',
        }, children: jsx("path", { d: "M2 4l4 4 4-4" }) }));
}
function SearchIcon({ large }) {
    const size = large ? 32 : 16;
    return (jsxs("svg", { width: size, height: size, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("circle", { cx: "11", cy: "11", r: "8" }), jsx("path", { d: "m21 21-4.35-4.35" })] }));
}
...
function ChevronLeftIcon() {
    return (jsx("svg", { width: "16", height: "16", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: jsx("polyline", { points: "15 18 9 12 15 6" }) }));
}
function ChevronRightIcon() {
    return (jsx("svg", { width: "16", height: "16", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: jsx("polyline", { points: "9 18 15 12 9 6" }) }));
}
// Apply appearance variables
...
function CreateOrganization({ onCreate, onCancel, redirectUrl, skipInvitationScreen = false, appearance, className, }) {
    ...
    // Invitation Screen
    if (createdOrg) {
        return (jsx("div", { style: applyAppearance$4(styles$7.container, appearance), className: className, children: jsxs("div", { style: styles$7.successState, children: [jsx("div", { style: styles$7.successIcon, children: "\u2713" }), jsx("h2", { style: applyAppearance$4(styles$7.title, appearance), children: "Organization Created!" }), jsxs("p", { style: styles$7.successText, children: [jsx("strong", { children: createdOrg.name }), " has been created successfully."] }), inviteSent ? (jsxs("div", { style: applyAppearance$4(styles$7.successMessage, appearance), children: ["Invitation sent to ", inviteEmail, "!"] })) : (jsxs("form", { onSubmit: handleSendInvite, style: styles$7.inviteForm, children: [jsx("p", { style: styles$7.inviteText, children: "Want to invite team members now?" }), error && (jsx("div", { style: applyAppearance$4(styles$7.error, appearance), role: "alert", children: error })), jsxs("div", { style: styles$7.field, children: [jsx("label", { htmlFor: "vault-invite-email", style: applyAppearance$4(styles$7.label, appearance), children: "Email Address" }), jsx("input", { id: "vault-invite-email", type: "email", value: inviteEmail, onChange: (e) => setInviteEmail(e.target.value), placeholder: "colleague@example.com", style: applyAppearance$4(styles$7.input, appearance), disabled: isLoading })] }), jsxs("div", { style: styles$7.field, children: [jsx("label", { htmlFor: "vault-invite-role", style: applyAppearance$4(styles$7.label, appearance), children: "Role" }), jsxs("select", { id: "vault-invite-role", value: inviteRole, onChange: (e) => setInviteRole(e.target.value), style: applyAppearance$4(styles$7.select, appearance), disabled: isLoading, children: [jsx("option", { value: "member", children: "Member" }), jsx("option", { value: "admin", children: "Admin" })] })] }), jsxs("div", { style: styles$7.buttonGroup, children: [jsx("button", { type: "submit", disabled: isLoading, style: applyAppearance$4(styles$7.primaryButton, appearance), children: isLoading ? 'Sending...' : 'Send Invitation' }), jsx("button", { type: "button", onClick: handleSkipInvite, style: applyAppearance$4(styles$7.secondaryButton, appearance), children: "Skip for now" })] })] }))] }) }));
    }
    // Create Organization Form
    return (jsx("div", { style: applyAppearance$4(styles$7.container, appearance), className: className, children: jsxs("form", { onSubmit: handleSubmit, style: styles$7.form, children: [jsx("h2", { style: applyAppearance$4(styles$7.title, appearance), children: "Create Organization" }), jsx("p", { style: styles$7.subtitle, children: "Set up a new organization to collaborate with your team." }), error && (jsx("div", { style: applyAppearance$4(styles$7.error, appearance), role: "alert", children: error })), jsxs("div", { style: styles$7.field, children: [jsx("label", { htmlFor: "vault-org-name", style: applyAppearance$4(styles$7.label, appearance), children: "Organization Name *" }), jsx("input", { id: "vault-org-name", type: "text", value: name, onChange: (e) => handleNameChange(e.target.value), placeholder: "Acme Inc", required: true, style: applyAppearance$4(styles$7.input, appearance), disabled: isLoading })] }), jsxs("div", { style: styles$7.field, children: [jsx("label", { htmlFor: "vault-org-slug", style: applyAppearance$4(styles$7.label, appearance), children: "Organization Slug *" }), jsx("input", { id: "vault-org-slug", type: "text", value: slug, onChange: (e) => setSlug(e.target.value), placeholder: "acme-inc", required: true, style: applyAppearance$4(styles$7.input, appearance), disabled: isLoading }), jsxs("span", { style: styles$7.helpText, children: ["Used in URLs: vault.dev/o/", slug || 'your-org'] })] }), jsxs("div", { style: styles$7.field, children: [jsx("label", { htmlFor: "vault-org-description", style: applyAppearance$4(styles$7.label, appearance), children: "Description" }), jsx("textarea", { id: "vault-org-description", value: description, onChange: (e) => setDescription(e.target.value), placeholder: "What does your organization do?", rows: 3, style: applyAppearance$4(styles$7.textarea, appearance), disabled: isLoading })] }), jsxs("div", { style: styles$7.field, children: [jsx("label", { htmlFor: "vault-org-logo", style: applyAppearance$4(styles$7.label, appearance), children: "Logo" }), jsx("div", { style: styles$7.logoUpload, children: logoPreview ? (jsxs("div", { style: styles$7.logoPreview, children: [jsx("img", { src: logoPreview, alt: "Logo preview", style: styles$7.logoImage }), jsx("button", { type: "button", onClick: () => {
                                            setLogo(null);
                                            setLogoPreview(null);
                                        }, style: styles$7.removeLogo, children: "\u2715" })] })) : (jsxs("label", { style: applyAppearance$4(styles$7.logoInputLabel, appearance), children: [jsx("input", { id: "vault-org-logo", type: "file", accept: "image/*", onChange: handleLogoChange, style: styles$7.fileInput, disabled: isLoading }), jsx(UploadIcon, {}), jsx("span", { children: "Upload logo" })] })) }), jsx("span", { style: styles$7.helpText, children: "Max 5MB, PNG or JPG recommended" })] }), jsxs("div", { style: styles$7.buttonGroup, children: [jsx("button", { type: "submit", disabled: isLoading || !name.trim() || !slug.trim(), style: applyAppearance$4(styles$7.primaryButton, appearance), children: isLoading ? 'Creating...' : 'Create Organization' }), onCancel && (jsx("button", { type: "button", onClick: onCancel, disabled: isLoading, style: applyAppearance$4(styles$7.secondaryButton, appearance), children: "Cancel" }))] })] }) }));
}
// Icon Components
function UploadIcon() {
    return (jsxs("svg", { width: "20", height: "20", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("path", { d: "M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" }), jsx("polyline", { points: "17 8 12 3 7 8" }), jsx("line", { x1: "12", y1: "3", x2: "12", y2: "15" })] }));
}
// Apply appearance variables
...
function OrganizationProfile({ organization: propOrg, appearance, className, }) {
    ...
    if (!isLoaded) {
        return (jsx("div", { style: applyAppearance$3(styles$6.container, appearance), className: className, children: jsx("div", { style: styles$6.loading, children: "Loading..." }) }));
    }
    if (!organization) {
        return (jsx("div", { style: applyAppearance$3(styles$6.container, appearance), className: className, children: jsxs("div", { style: styles$6.emptyState, children: [jsx("p", { children: "No organization selected." }), jsx("p", { style: styles$6.emptyStateSubtext, children: "Create or join an organization to manage it here." })] }) }));
    }
    const isOwner = organization.role === 'owner';
    const isAdmin = organization.role === 'admin' || isOwner;
    return (jsxs("div", { style: applyAppearance$3(styles$6.container, appearance), className: className, children: [jsx("div", { style: styles$6.header, children: jsxs("div", { children: [jsx("h1", { style: applyAppearance$3(styles$6.heading, appearance), children: organization.name }), jsx("span", { style: styles$6.roleBadge, children: organization.role })] }) }), jsxs("div", { style: styles$6.tabs, children: [jsx(TabButton, { label: "General", isActive: activeTab === 'general', onClick: () => setActiveTab('general'), appearance: appearance }), jsx(TabButton, { label: "Members", isActive: activeTab === 'members', onClick: () => setActiveTab('members'), appearance: appearance }), jsx(TabButton, { label: "Settings", isActive: activeTab === 'settings', onClick: () => setActiveTab('settings'), appearance: appearance })] }), error && (jsx("div", { style: applyAppearance$3(styles$6.error, appearance), role: "alert", children: error })), successMessage && (jsx("div", { style: applyAppearance$3(styles$6.success, appearance), role: "status", children: successMessage })), activeTab === 'general' && (jsxs("div", { style: styles$6.section, children: [jsxs("div", { style: styles$6.sectionHeader, children: [jsx("h2", { style: styles$6.sectionTitle, children: "Organization Details" }), isAdmin && !isEditing && (jsx("button", { onClick: () => setIsEditing(true), style: applyAppearance$3(styles$6.editButton, appearance), children: "Edit" }))] }), jsxs("div", { style: styles$6.fieldGroup, children: [jsxs("div", { style: styles$6.field, children: [jsx("label", { style: applyAppearance$3(styles$6.label, appearance), children: "Organization Name" }), jsx("input", { type: "text", value: name, onChange: (e) => setName(e.target.value), disabled: !isEditing || isSaving, style: applyAppearance$3(styles$6.input, appearance) })] }), jsxs("div", { style: styles$6.field, children: [jsx("label", { style: applyAppearance$3(styles$6.label, appearance), children: "Organization Slug" }), jsx("input", { type: "text", value: slug, onChange: (e) => setSlug(e.target.value), disabled: !isEditing || isSaving, style: applyAppearance$3(styles$6.input, appearance) }), jsxs("span", { style: styles$6.helpText, children: ["Used in URLs: vault.dev/o/", slug] })] }), jsxs("div", { style: styles$6.field, children: [jsx("label", { style: applyAppearance$3(styles$6.label, appearance), children: "Description" }), jsx("textarea", { value: description, onChange: (e) => setDescription(e.target.value), disabled: !isEditing || isSaving, rows: 3, style: applyAppearance$3(styles$6.textarea, appearance), placeholder: "What does your organization do?" })] }), organization.logoUrl && (jsxs("div", { style: styles$6.field, children: [jsx("label", { style: applyAppearance$3(styles$6.label, appearance), children: "Logo" }), jsx("img", { src: organization.logoUrl, alt: "Organization logo", style: styles$6.logoImage })] }))] }), isEditing && (jsxs("div", { style: styles$6.buttonGroup, children: [jsx("button", { onClick: handleSaveGeneral, disabled: isSaving || !name.trim() || !slug.trim(), style: applyAppearance$3(styles$6.primaryButton, appearance), children: isSaving ? 'Saving...' : 'Save Changes' }), jsx("button", { onClick: () => {
                                    setIsEditing(false);
                                    setName(organization.name);
                                    setSlug(organization.slug);
                                    setDescription(organization.description || '');
                                    setError(null);
                                }, disabled: isSaving, style: applyAppearance$3(styles$6.secondaryButton, appearance), children: "Cancel" })] }))] })), activeTab === 'members' && (jsxs("div", { style: styles$6.section, children: [jsx("h2", { style: styles$6.sectionTitle, children: "Members" }), isAdmin && (jsx("form", { onSubmit: handleInviteMember, style: styles$6.inviteForm, children: jsxs("div", { style: styles$6.inviteRow, children: [jsx("input", { type: "email", value: inviteEmail, onChange: (e) => setInviteEmail(e.target.value), placeholder: "colleague@example.com", style: { ...applyAppearance$3(styles$6.input, appearance), flex: 1 }, disabled: isInviting }), jsxs("select", { value: inviteRole, onChange: (e) => setInviteRole(e.target.value), style: applyAppearance$3(styles$6.select, appearance), disabled: isInviting, children: [jsx("option", { value: "member", children: "Member" }), jsx("option", { value: "admin", children: "Admin" }), isOwner && jsx("option", { value: "owner", children: "Owner" })] }), jsx("button", { type: "submit", disabled: isInviting || !inviteEmail.trim(), style: applyAppearance$3(styles$6.primaryButton, appearance), children: isInviting ? 'Inviting...' : 'Invite' })] }) })), isLoadingMembers ? (jsx("div", { style: styles$6.loading, children: "Loading members..." })) : (jsx("div", { style: styles$6.membersList, children: members.length === 0 ? (jsx("div", { style: styles$6.emptyMembers, children: "No members yet." })) : (members.map((member) => (jsxs("div", { style: styles$6.memberRow, children: [jsxs("div", { style: styles$6.memberInfo, children: [jsx("div", { style: styles$6.memberAvatar, children: member.name?.[0] || member.email[0].toUpperCase() }), jsxs("div", { children: [jsx("div", { style: styles$6.memberName, children: member.name || member.email }), member.name && (jsx("div", { style: styles$6.memberEmail, children: member.email }))] })] }), jsx("div", { style: styles$6.memberActions, children: isAdmin && member.userId !== vault.user?.id ? (jsxs(Fragment, { children: [jsxs("select", { value: member.role, onChange: (e) => handleUpdateMemberRole(member.id, e.target.value), style: applyAppearance$3(styles$6.roleSelect, appearance), children: [jsx("option", { value: "member", children: "Member" }), jsx("option", { value: "admin", children: "Admin" }), isOwner && jsx("option", { value: "owner", children: "Owner" })] }), jsx("button", { onClick: () => handleRemoveMember(member.id), style: styles$6.removeButton, title: "Remove member", children: "\u2715" })] })) : (jsx("span", { style: styles$6.roleBadge, children: member.role })) })] }, member.id)))) }))] })), activeTab === 'settings' && (jsxs("div", { style: styles$6.section, children: [jsx("h2", { style: styles$6.sectionTitle, children: "Organization Settings" }), !isOwner && (jsxs("div", { style: styles$6.dangerBox, children: [jsx("h3", { style: styles$6.dangerTitle, children: "Leave Organization" }), jsx("p", { style: styles$6.dangerText, children: "You will lose access to all resources in this organization." }), jsx("button", { onClick: handleLeaveOrganization, style: applyAppearance$3(styles$6.dangerButton, appearance), children: "Leave Organization" })] })), isOwner && (jsxs("div", { style: styles$6.dangerBox, children: [jsx("h3", { style: styles$6.dangerTitle, children: "Delete Organization" }), jsx("p", { style: styles$6.dangerText, children: "Once deleted, this organization and all its data cannot be recovered. This action cannot be undone." }), !showDeleteConfirm ? (jsx("button", { onClick: () => setShowDeleteConfirm(true), style: applyAppearance$3(styles$6.dangerButton, appearance), children: "Delete Organization" })) : (jsxs("div", { style: styles$6.confirmDelete, children: [jsxs("p", { style: styles$6.confirmText, children: ["Type ", jsx("strong", { children: organization.name }), " to confirm:"] }), jsx("input", { type: "text", value: deleteConfirmText, onChange: (e) => setDeleteConfirmText(e.target.value), placeholder: organization.name, style: applyAppearance$3(styles$6.input, appearance) }), jsxs("div", { style: styles$6.buttonGroup, children: [jsx("button", { onClick: handleDeleteOrganization, disabled: isDeleting || deleteConfirmText !== organization.name, style: applyAppearance$3(styles$6.dangerButton, appearance), children: isDeleting ? 'Deleting...' : 'Confirm Delete' }), jsx("button", { onClick: () => {
...
function OrganizationList({ onSelect, hideCreateButton = false, appearance, className, }) {
    ...
    if (!isLoaded) {
        return (jsx("div", { style: applyAppearance$2(styles$5.container, appearance), className: className, children: jsx("div", { style: styles$5.loading, children: "Loading organizations..." }) }));
    }
    const hasOrganizations = organizations.length > 0;
    return (jsxs("div", { style: applyAppearance$2(styles$5.container, appearance), className: className, children: [jsxs("div", { style: styles$5.header, children: [jsx("h2", { style: applyAppearance$2(styles$5.title, appearance), children: "Your Organizations" }), !hideCreateButton && !isCreating && (jsxs("button", { onClick: () => setIsCreating(true), style: applyAppearance$2(styles$5.createButton, appearance), children: [jsx(PlusIcon, {}), jsx("span", { children: "New" })] }))] }), error && (jsx("div", { style: applyAppearance$2(styles$5.error, appearance), role: "alert", children: error })), successMessage && (jsx("div", { style: applyAppearance$2(styles$5.success, appearance), role: "status", children: successMessage })), isCreating && (jsxs("form", { onSubmit: handleCreateOrg, style: styles$5.createForm, children: [jsx("input", { type: "text", value: newOrgName, onChange: (e) => setNewOrgName(e.target.value), placeholder: "Organization name", autoFocus: true, style: applyAppearance$2(styles$5.createInput, appearance), disabled: isLoading }), jsxs("div", { style: styles$5.createActions, children: [jsx("button", { type: "submit", disabled: isLoading || !newOrgName.trim(), style: applyAppearance$2(styles$5.primaryButton, appearance), children: isLoading ? 'Creating...' : 'Create' }), jsx("button", { type: "button", onClick: () => {
                                    setIsCreating(false);
                                    setNewOrgName('');
                                    setError(null);
                                }, disabled: isLoading, style: applyAppearance$2(styles$5.secondaryButton, appearance), children: "Cancel" })] })] })), jsxs("div", { style: styles$5.list, children: [jsx(OrganizationItem, { name: user?.profile?.name || user?.email || 'Personal Account', email: user?.email, isActive: !activeOrg, onClick: () => {
                            setActive(null);
                            onSelect?.(null);
                        }, isPersonal: true, appearance: appearance }), organizations.map((org) => (jsx(OrganizationItem, { name: org.name, role: org.role, isActive: activeOrg?.id === org.id, onClick: () => handleSelectOrg(org), appearance: appearance }, org.id))), !hasOrganizations && !isCreating && (jsxs("div", { style: styles$5.emptyState, children: [jsx("div", { style: styles$5.emptyIcon, children: jsx(BuildingIcon, {}) }), jsx("p", { style: styles$5.emptyTitle, children: "No organizations yet" }), jsx("p", { style: styles$5.emptyText, children: "Create an organization to collaborate with your team." }), !hideCreateButton && (jsx("button", { onClick: () => setIsCreating(true), style: applyAppearance$2(styles$5.emptyCreateButton, appearance), children: "Create Organization" }))] }))] })] }));
}
function OrganizationItem({ name, email, role, isActive, onClick, isPersonal = false, appearance, }) {
    return (jsxs("button", { onClick: onClick, style: {
            ...styles$5.item,
        ...
        }, children: [jsx("div", { style: styles$5.itemIcon, children: isPersonal ? jsx(UserIcon$1, {}) : jsx(BuildingIcon, { small: true }) }), jsxs("div", { style: styles$5.itemContent, children: [jsx("div", { style: styles$5.itemName, children: name }), email && jsx("div", { style: styles$5.itemEmail, children: email }), role && !isPersonal && (jsx("span", { style: styles$5.itemRole, children: role }))] }), isActive && (jsx("div", { style: {
...
function BuildingIcon({ small }) {
    return (jsxs("svg", { width: small ? 20 : 24, height: small ? 20 : 24, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("path", { d: "M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18Z" }), jsx("path", { d: "M6 12H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2" }), jsx("path", { d: "M18 9h2a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-2" }), jsx("path", { d: "M10 6h4" }), jsx("path", { d: "M10 10h4" }), jsx("path", { d: "M10 14h4" }), jsx("path", { d: "M10 18h4" })] }));
}
function UserIcon$1() {
    return (jsxs("svg", { width: 20, height: 20, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("path", { d: "M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" }), jsx("circle", { cx: "12", cy: "7", r: "4" })] }));
}
function PlusIcon() {
    return (jsxs("svg", { width: 16, height: 16, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("path", { d: "M5 12h14" }), jsx("path", { d: "M12 5v14" })] }));
}
...
function SessionManagement({ appearance, className, }) {
    ...
    return (jsxs("div", { style: applyAppearance$1(styles$4.container, appearance), className: className, children: [jsx("div", { style: styles$4.header, children: jsxs("div", { children: [jsx("h2", { style: applyAppearance$1(styles$4.title, appearance), children: "Active Sessions" }), jsx("p", { style: styles$4.subtitle, children: "Manage your active sessions across all devices" })] }) }), otherSessionsCount > 0 && (jsxs("div", { style: styles$4.warningBanner, children: [jsx(AlertIcon, {}), jsxs("span", { children: ["You have ", otherSessionsCount, " other active session", otherSessionsCount !== 1 ? 's' : ''] })] })), showConfirmAll ? (jsxs("div", { style: styles$4.confirmBanner, children: [jsx("p", { style: styles$4.confirmText, children: "Are you sure you want to sign out all other devices? This will require you to sign in again on those devices." }), jsxs("div", { style: styles$4.confirmActions, children: [jsx("button", { onClick: handleRevokeAllOthers, disabled: isLoading, style: applyAppearance$1(styles$4.dangerButton, appearance), children: isLoading ? 'Signing out...' : 'Yes, sign out all' }), jsx("button", { onClick: () => setShowConfirmAll(false), disabled: isLoading, style: applyAppearance$1(styles$4.secondaryButton, appearance), children: "Cancel" })] })] })) : (otherSessionsCount > 0 && (jsxs("button", { onClick: () => setShowConfirmAll(true), style: applyAppearance$1(styles$4.signOutAllButton, appearance), children: [jsx(LogOutIcon$1, {}), "Sign out all other devices"] }))), jsx("div", { style: styles$4.sessionsList, children: sessions.length === 0 ? (jsx("div", { style: styles$4.emptyState, children: jsx("p", { children: "No active sessions found" }) })) : (sessions
                    .sort((a, b) => (a.isCurrent ? -1 : 1))
                    .map((session) => (jsx(SessionItem, { session: session, isCurrent: session.isCurrent, onRevoke: () => handleRevoke(session.id), isRevoking: revokingId === session.id, appearance: appearance }, session.id)))) })] }));
}
function SessionItem({ session, isCurrent, onRevoke, isRevoking, appearance }) {
    ...
    return (jsxs("div", { style: {
        ...
        }, children: [jsx("div", { style: styles$4.sessionIcon, children: jsx(DeviceIcon, { device: deviceInfo.device }) }), jsxs("div", { style: styles$4.sessionInfo, children: [jsxs("div", { style: styles$4.sessionHeader, children: [jsxs("span", { style: styles$4.deviceName, children: [deviceInfo.browser, " on ", deviceInfo.os] }), isCurrent && (jsx("span", { style: {
                                    ...styles$4.currentBadge,
                                    ...(appearance?.variables?.['colorPrimary'] && {
                                        backgroundColor: appearance.variables['colorPrimary'],
                                    }),
                                }, children: "Current" }))] }), jsxs("div", { style: styles$4.sessionDetails, children: [location && jsx("span", { children: location }), jsxs("span", { children: ["Last active ", lastActive] })] }), jsxs("div", { style: styles$4.sessionMeta, children: ["Started ", new Date(session.createdAt).toLocaleDateString()] })] }), !isCurrent && (jsx("button", { onClick: onRevoke, disabled: isRevoking, style: styles$4.revokeButton, title: "Sign out this device", children: isRevoking ? (jsx(Spinner$2, {})) : (jsx(LogOutIcon$1, { small: true })) }))] }));
}
// Helper Functions
...
function DeviceIcon({ device }) {
    if (device === 'mobile') {
        return (jsxs("svg", { width: "24", height: "24", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [jsx("rect", { x: "7", y: "2", width: "10", height: "20", rx: "2", ry: "2" }), jsx("line", { x1: "12", y1: "18", x2: "12", y2: "18" })] }));
    }
    if (device === 'tablet') {
        return (jsxs("svg", { width: "24", height: "24", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [jsx("rect", { x: "4", y: "2", width: "16", height: "20", rx: "2", ry: "2" }), jsx("line", { x1: "12", y1: "18", x2: "12", y2: "18" })] }));
    }
    return (jsxs("svg", { width: "24", height: "24", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [jsx("rect", { x: "2", y: "3", width: "20", height: "14", rx: "2", ry: "2" }), jsx("line", { x1: "8", y1: "21", x2: "16", y2: "21" }), jsx("line", { x1: "12", y1: "17", x2: "12", y2: "21" })] }));
}
function LogOutIcon$1({ small }) {
    const size = small ? 16 : 18;
    return (jsxs("svg", { width: size, height: size, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [jsx("path", { d: "M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" }), jsx("polyline", { points: "16 17 21 12 16 7" }), jsx("line", { x1: "21", y1: "12", x2: "9", y2: "12" })] }));
}
function AlertIcon() {
    return (jsxs("svg", { width: "16", height: "16", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", children: [jsx("path", { d: "M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" }), jsx("line", { x1: "12", y1: "9", x2: "12", y2: "13" }), jsx("line", { x1: "12", y1: "17", x2: "12.01", y2: "17" })] }));
}
function Spinner$2() {
    return (jsx("div", { style: styles$4.spinner, children: jsx("div", { style: styles$4.spinnerInner }) }));
}
// Apply appearance variables
...
function Waitlist({ onSubmit, redirectUrl, appearance, socialProof, className, }) {
    ...
    if (isSuccess) {
        return (jsx("div", { style: applyAppearance(styles$3.container, appearance), className: className, children: jsxs("div", { style: styles$3.successState, children: [jsx("div", { style: styles$3.successIcon, children: jsx(CheckIcon, {}) }), jsx("h2", { style: applyAppearance(styles$3.successTitle, appearance), children: "You're on the list!" }), jsxs("p", { style: styles$3.successText, children: ["We've added ", jsx("strong", { children: email }), " to our waitlist. We'll notify you when spots become available."] }), redirectUrl && (jsx("p", { style: styles$3.redirectText, children: "Redirecting you..." }))] }) }));
    }
    return (jsxs("div", { style: applyAppearance(styles$3.container, appearance), className: className, children: [jsxs("div", { style: styles$3.header, children: [jsx("h2", { style: applyAppearance(styles$3.title, appearance), children: "Join the Waitlist" }), jsx("p", { style: styles$3.subtitle, children: "Be the first to know when we launch. No spam, ever." })] }), socialProof && (jsxs("div", { style: styles$3.socialProof, children: [jsx(UsersIcon, {}), jsx("span", { children: socialProof })] })), jsxs("form", { onSubmit: handleSubmit, style: styles$3.form, children: [error && (jsx("div", { style: applyAppearance(styles$3.error, appearance), role: "alert", children: error })), jsxs("div", { style: styles$3.inputGroup, children: [jsx("input", { id: "vault-waitlist-email", type: "email", value: email, onChange: (e) => setEmail(e.target.value), placeholder: "Enter your email", required: true, disabled: isLoading, style: applyAppearance(styles$3.input, appearance), "aria-label": "Email address" }), jsx("button", { type: "submit", disabled: isLoading || !email.trim(), style: applyAppearance(styles$3.submitButton, appearance), children: isLoading ? (jsxs(Fragment, { children: [jsx(Spinner$1, {}), jsx("span", { children: "Joining..." })] })) : (jsxs(Fragment, { children: [jsx("span", { children: "Join Waitlist" }), jsx(ArrowRightIcon, {})] })) })] }), jsx("p", { style: styles$3.privacy, children: "By joining, you agree to our Terms of Service and Privacy Policy." })] })] }));
}
// Icon Components
function CheckIcon() {
    return (jsx("svg", { width: "32", height: "32", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "3", strokeLinecap: "round", strokeLinejoin: "round", children: jsx("polyline", { points: "20 6 9 17 4 12" }) }));
}
function UsersIcon() {
    return (jsxs("svg", { width: "16", height: "16", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("path", { d: "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" }), jsx("circle", { cx: "9", cy: "7", r: "4" }), jsx("path", { d: "M23 21v-2a4 4 0 0 0-3-3.87" }), jsx("path", { d: "M16 3.13a4 4 0 0 1 0 7.75" })] }));
}
function ArrowRightIcon() {
    return (jsxs("svg", { width: "16", height: "16", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("line", { x1: "5", y1: "12", x2: "19", y2: "12" }), jsx("polyline", { points: "12 5 19 12 12 19" })] }));
}
function Spinner$1() {
    return (jsx("div", { style: styles$3.spinner, children: jsx("div", { style: styles$3.spinnerInner }) }));
}
// Apply appearance variables
...
function ImpersonationBanner({ onStopImpersonating, }) {
    ...
    if (isDismissed) {
        return (jsx("button", { onClick: () => setIsDismissed(false), style: styles$2.showButton, title: "Show impersonation banner", children: jsx(UserIcon, {}) }));
    }
    return (jsxs("div", { style: styles$2.banner, role: "alert", "aria-live": "polite", children: [jsxs("div", { style: styles$2.content, children: [jsx("div", { style: styles$2.icon, children: jsx(UserIcon, {}) }), jsxs("div", { style: styles$2.text, children: [jsx("span", { style: styles$2.label, children: "Impersonating" }), jsx("span", { style: styles$2.userInfo, children: user?.profile?.name || user?.email || 'Unknown User' })] })] }), jsxs("div", { style: styles$2.actions, children: [jsx("button", { onClick: handleStopImpersonating, disabled: isStopping, style: styles$2.stopButton, children: isStopping ? (jsxs(Fragment, { children: [jsx(Spinner, {}), jsx("span", { children: "Stopping..." })] })) : (jsxs(Fragment, { children: [jsx(LogOutIcon, {}), jsx("span", { children: "Stop Impersonating" })] })) }), jsx("button", { onClick: () => setIsDismissed(true), style: styles$2.dismissButton, title: "Dismiss banner", "aria-label": "Dismiss impersonation banner", children: jsx(CloseIcon, {}) })] })] }));
}
// Icon Components
function UserIcon() {
    return (jsxs("svg", { width: "18", height: "18", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("path", { d: "M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" }), jsx("circle", { cx: "12", cy: "7", r: "4" })] }));
}
function LogOutIcon() {
    return (jsxs("svg", { width: "14", height: "14", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("path", { d: "M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" }), jsx("polyline", { points: "16 17 21 12 16 7" }), jsx("line", { x1: "21", y1: "12", x2: "9", y2: "12" })] }));
}
function CloseIcon() {
...
function Spinner() {
    return (jsx("div", { style: styles$2.spinner, children: jsx("div", { style: styles$2.spinnerInner }) }));
}
// Styles
const styles$2 = {
    banner: {
...
function SignedIn({ children }) {
    ...
    return isSignedIn ? jsx(Fragment, { children: children }) : null;
}
/**
...
function RequireAuth({ children, fallback, loading }) {
    ...
    if (!isLoaded) {
        ...
        return (jsxs("div", { style: styles$1.loading, children: [jsx("div", { style: styles$1.spinner }), jsx("span", { children: "Loading..." })] }));
    }
    if (!isSignedIn) {
        if (fallback) {
            return jsx(Fragment, { children: fallback });
        }
        return (jsxs("div", { style: styles$1.unauthenticated, children: [jsx("p", { children: "Please sign in to continue." }), jsx("a", { href: "/sign-in", style: styles$1.link, children: "Sign In" })] }));
    }
...
function Protect({ children, fallback, role, permission, loading, }) {
    ...
    if (!isLoaded) {
        ...
        return (jsxs("div", { style: styles.loading, children: [jsx("div", { style: styles.spinner }), jsx("span", { children: "Loading..." })] }));
    }
    // Not signed in
    if (!isSignedIn || !user) {
        if (fallback) {
        ...
        return (jsxs("div", { style: styles.unauthorized, children: [jsx("h2", { style: styles.title, children: "Sign in required" }), jsx("p", { style: styles.message, children: "You need to be signed in to access this page." }), jsx("a", { href: "/sign-in", style: styles.link, children: "Sign In" })] }));
    }
    // Check role requirement
    if (role) {
        const userRole = organization?.role;
        if (userRole !== role && userRole !== 'owner') {
            return (jsxs("div", { style: styles.unauthorized, children: [jsx("h2", { style: styles.title, children: "Access denied" }), jsx("p", { style: styles.message, children: "You don't have the required role to access this page." }), jsx("a", { href: "/", style: styles.link, children: "Go Home" })] }));
        }
    }
...
function RedirectToSignIn({ redirectUrl }) {
    ...
    return (jsxs("div", { style: styles.loading, children: [jsx("div", { style: styles.spinner }), jsx("span", { children: "Redirecting..." })] }));
}
/**
...
const PricingTable = ({ plans, currentPlanId, currentInterval = 'month', loading = false, onSelectPlan, onIntervalChange, showFeatures = true, className = '', }) => {
    ...
    // Loading state
    if (loading) {
        return (jsx("div", { className: `vault-pricing-table vault-pricing-loading ${className}`, children: jsx("div", { className: "vault-pricing-skeleton", children: [1, 2, 3].map((i) => (jsx("div", { className: "vault-pricing-card-skeleton" }, i))) }) }));
    }
    // Empty state
    if (!plans || plans.length === 0) {
        return (jsx("div", { className: `vault-pricing-table vault-pricing-empty ${className}`, children: jsx("p", { children: "No plans available" }) }));
    }
    return (jsxs("div", { className: `vault-pricing-table ${className}`, style: {
        ...
        }, children: [onIntervalChange && (jsxs("div", { className: "vault-pricing-interval-toggle", children: [jsx("button", { className: `vault-interval-btn ${selectedInterval === 'month' ? 'active' : ''}`, onClick: () => handleIntervalChange('month'), type: "button", children: "Monthly" }), jsxs("button", { className: `vault-interval-btn ${selectedInterval === 'year' ? 'active' : ''}`, onClick: () => handleIntervalChange('year'), type: "button", children: ["Yearly", jsx("span", { className: "vault-save-badge", children: "Save 20%" })] })] })), jsx("div", { className: "vault-pricing-grid", children: plans.map((plan) => {
                    const features = getFeatures(plan);
                    const isCurrent = isCurrentPlan(plan);
                    return (jsxs("div", { className: `vault-pricing-card ${isCurrent ? 'current' : ''}`, "data-plan-id": plan.id, children: [isCurrent && (jsx("div", { className: "vault-current-plan-badge", children: "Current Plan" })), jsxs("div", { className: "vault-pricing-header", children: [jsx("h3", { className: "vault-plan-name", children: plan.name }), plan.description && (jsx("p", { className: "vault-plan-description", children: plan.description }))] }), jsxs("div", { className: "vault-pricing-price", children: [jsx("span", { className: "vault-price-amount", children: formatPrice(plan.amount, plan.currency) }), jsxs("span", { className: "vault-price-interval", children: ["/", plan.interval] })] }), showFeatures && features.length > 0 && (jsx("ul", { className: "vault-pricing-features", children: features.map((feature, index) => (jsxs("li", { className: `vault-feature-item ${feature.included ? 'included' : 'excluded'}`, title: feature.description, children: [jsx("svg", { className: "vault-feature-icon", viewBox: "0 0 20 20", fill: "currentColor", children: feature.included ? (jsx("path", { fillRule: "evenodd", d: "M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z", clipRule: "evenodd" })) : (jsx("path", { fillRule: "evenodd", d: "M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z", clipRule: "evenodd" })) }), jsx("span", { className: "vault-feature-name", children: feature.name })] }, index))) })), onSelectPlan && (jsx("button", { className: `vault-pricing-cta ${isCurrent ? 'current' : ''}`, onClick: () => onSelectPlan(plan), disabled: isCurrent, type: "button", children: isCurrent ? 'Current Plan' : 'Subscribe' }))] }, plan.id));
                }) }), jsx("style", { children: `
        .vault-pricing-table {
...
const CheckoutButton = ({ priceId, successUrl, cancelUrl, children, disabled = false, loading: externalLoading = false, onCheckout, onError, className = '', }) => {
    ...
    return (jsxs("button", { className: `vault-checkout-button ${isLoading ? 'loading' : ''} ${className}`, onClick: handleClick, disabled: isDisabled, type: "button", style: {
            '--vault-primary': appearance?.variables?.colorPrimary,
            '--vault-background': appearance?.variables?.colorBackground,
            '--vault-foreground': appearance?.variables?.colorText,
        }, children: [isLoading ? (jsxs(Fragment, { children: [jsx("span", { className: "vault-checkout-spinner" }), jsx("span", { children: "Loading..." })] })) : (children), jsx("style", { children: `
...
const QuickCheckoutButton = ({ priceId, children, successPath = '/billing/success', cancelPath = '/billing/cancel', ...props }) => {
    ...
    const cancelUrl = typeof window !== 'undefined'
        ...
        : cancelPath;
    return (jsx(CheckoutButton, { priceId: priceId, successUrl: successUrl, cancelUrl: cancelUrl, ...props, children: children }));
};
...
const CustomerPortalButton = ({ children, returnUrl: propReturnUrl, disabled = false, onOpen, onError, className = '', }) => {
    ...
    return (jsxs("button", { className: `vault-portal-button ${isLoading ? 'loading' : ''} ${className}`, onClick: handleClick, disabled: disabled || isLoading, type: "button", style: {
            '--vault-primary': appearance?.variables?.colorPrimary,
            '--vault-background': appearance?.variables?.colorBackground,
            '--vault-foreground': appearance?.variables?.colorText,
        }, children: [isLoading ? (jsxs(Fragment, { children: [jsx("span", { className: "vault-portal-spinner" }), jsx("span", { children: "Loading..." })] })) : (children), jsx("style", { children: `
        .vault-portal-button {
          --vault-primary-color: var(--vault-primary, #3b82f6);
          --vault-text-primary: var(--vault-foreground, #111827);
          --vault-card-bg: var(--vault-background, #ffffff);
...
const ManageSubscriptionButton = (props) => {
    return jsx(CustomerPortalButton, { ...props });
};
/**
...
const BillingSettings = ({ returnUrl, showManageSubscription = true, showUpdatePayment = true, showViewInvoices = true, className = '', }) => {
    const { appearance } = useTheme();
    return (jsxs("div", { className: `vault-billing-settings ${className}`, style: {
            '--vault-primary': appearance?.variables?.colorPrimary,
            '--vault-background': appearance?.variables?.colorBackground,
            '--vault-foreground': appearance?.variables?.colorText,
        }, children: [jsx("h3", { className: "vault-billing-settings-title", children: "Billing Settings" }), jsxs("div", { className: "vault-billing-settings-list", children: [showManageSubscription && (jsxs("div", { className: "vault-billing-setting-item", children: [jsxs("div", { className: "vault-billing-setting-info", children: [jsx("h4", { children: "Subscription Plan" }), jsx("p", { children: "Upgrade, downgrade, or cancel your subscription" })] }), jsx(CustomerPortalButton, { returnUrl: returnUrl, children: "Manage" })] })), showUpdatePayment && (jsxs("div", { className: "vault-billing-setting-item", children: [jsxs("div", { className: "vault-billing-setting-info", children: [jsx("h4", { children: "Payment Methods" }), jsx("p", { children: "Add or update your payment methods" })] }), jsx(UpdatePaymentMethodButton, { returnUrl: returnUrl, children: "Update" })] })), showViewInvoices && (jsxs("div", { className: "vault-billing-setting-item", children: [jsxs("div", { className: "vault-billing-setting-info", children: [jsx("h4", { children: "Billing History" }), jsx("p", { children: "View and download your invoices" })] }), jsx(ViewInvoicesButton, { returnUrl: returnUrl, children: "View" })] }))] }), jsx("style", { children: `
...
const SubscriptionStatus = ({ subscription: propSubscription, showDetails = true, showInvoices = false, showUsage = false, onCancel, onResume, onUpdate, className = '', }) => {
    ...
    // Loading state
    if (isLoading) {
        return (jsx("div", { className: `vault-subscription-status loading ${className}`, children: jsx("div", { className: "vault-subscription-skeleton" }) }));
    }
    // No subscription state
    if (!subscription) {
        return (jsxs("div", { className: `vault-subscription-status empty ${className}`, style: {
                '--vault-primary': appearance?.variables?.colorPrimary,
                '--vault-background': appearance?.variables?.colorBackground,
                '--vault-foreground': appearance?.variables?.colorText,
            }, children: [jsxs("div", { className: "vault-subscription-empty", children: [jsx("svg", { className: "vault-empty-icon", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", children: jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" }) }), jsx("h3", { children: "No Active Subscription" }), jsx("p", { children: "You don't have an active subscription. Choose a plan to get started." })] }), jsx("style", { children: `
    ...
    return (jsxs("div", { className: `vault-subscription-status ${className}`, style: {
            '--vault-primary': appearance?.variables?.colorPrimary,
            '--vault-background': appearance?.variables?.colorBackground,
            '--vault-foreground': appearance?.variables?.colorText,
        }, children: [jsxs("div", { className: "vault-subscription-header", children: [jsxs("div", { className: "vault-subscription-info", children: [jsx("h3", { className: "vault-plan-name", children: subscription.plan?.name || 'Subscription' }), jsx("span", { className: `vault-status-badge ${getStatusColor(subscription.status)}`, children: subscription.status.replace('_', ' ') })] }), subscription.plan && (jsxs("div", { className: "vault-plan-price", children: [jsxs("span", { className: "vault-price-amount", children: ["$", (subscription.plan.amount / 100).toFixed(2)] }), jsxs("span", { className: "vault-price-interval", children: ["/", subscription.plan.interval] })] }))] }), showDetails && (jsxs("div", { className: "vault-subscription-details", children: [isTrialing && daysLeftInTrial !== null && (jsxs("div", { className: "vault-detail-item vault-trial-info", children: [jsx("span", { className: "vault-detail-label", children: "Trial ends in" }), jsxs("span", { className: "vault-detail-value vault-trial-days", children: [daysLeftInTrial, " days"] })] })), isActive && !isCanceled && (jsxs("div", { className: "vault-detail-item", children: [jsx("span", { className: "vault-detail-label", children: willRenew ? 'Next billing date' : 'Access until' }), jsx("span", { className: "vault-detail-value", children: formatDate(subscription.currentPeriodEnd) })] })), isCanceled && (jsxs("div", { className: "vault-detail-item vault-canceled-info", children: [jsx("span", { className: "vault-detail-label", children: "Subscription ends" }), jsx("span", { className: "vault-detail-value", children: formatDate(subscription.currentPeriodEnd) })] })), subscription.cancelAtPeriodEnd && !isCanceled && (jsxs("div", { className: "vault-cancel-warning", children: [jsx("svg", { viewBox: "0 0 20 20", fill: "currentColor", children: jsx("path", { fillRule: "evenodd", d: "M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z", clipRule: "evenodd" }) }), jsx("span", { children: "Your subscription will not renew" })] }))] })), showUsage && usage && (jsxs("div", { className: "vault-usage-section", children: [jsx("h4", { children: "Usage This Period" }), jsx(UsageMeter, { usage: usage, showPercentage: true })] })), jsxs("div", { className: "vault-subscription-actions", children: [onCancel && isActive && !subscription.cancelAtPeriodEnd && (jsx("button", { className: "vault-action-btn cancel", onClick: handleCancel, disabled: isActionLoading, type: "button", children: isActionLoading ? 'Processing...' : 'Cancel Subscription' })), onResume && subscription.cancelAtPeriodEnd && (jsx("button", { className: "vault-action-btn resume", onClick: handleResume, disabled: isActionLoading, type: "button", children: isActionLoading ? 'Processing...' : 'Resume Subscription' }))] }), jsx("style", { children: `
...
const UsageMeter = ({ usage, showPercentage = true, showRemaining = false, className = '', }) => {
    ...
    return (jsxs("div", { className: `vault-usage-meter ${className} ${isOverLimit ? 'over-limit' : ''} ${isNearLimit ? 'near-limit' : ''}`, style: {
            '--vault-primary': appearance?.variables?.colorPrimary,
            '--vault-foreground': appearance?.variables?.colorText,
        }, children: [jsx("div", { className: "vault-usage-bar-container", children: jsx("div", { className: "vault-usage-bar", style: {
                        width: `${percentage}%`,
                        backgroundColor: getBarColor(),
                    } }) }), jsxs("div", { className: "vault-usage-info", children: [showPercentage && (jsxs("span", { className: "vault-usage-percentage", children: [percentage.toFixed(0), "%"] })), jsxs("span", { className: "vault-usage-values", children: [usage.totalUsage.toLocaleString(), usage.quota && ` / ${usage.quota.limit.toLocaleString()}`, " ", usage.metric] }), showRemaining && remaining > 0 && (jsxs("span", { className: "vault-usage-remaining", children: ["(", remaining.toLocaleString(), " remaining)"] }))] }), isOverLimit && (jsx("div", { className: "vault-usage-warning error", children: "You've exceeded your limit. Please upgrade your plan." })), isNearLimit && !isOverLimit && (jsx("div", { className: "vault-usage-warning warning", children: "You're approaching your usage limit." })), jsx("style", { children: `
        .vault-usage-meter {
          --vault-text-primary: var(--vault-foreground, #111827);
          --vault-text-secondary: #6b7280;
          --vault-primary-color: var(--vault-primary, #3b82f6);
...
const InvoiceList = ({ invoices, loading = false, emptyMessage = 'No invoices yet', className = '', }) => {
    ...
    if (loading) {
        return (jsx("div", { className: `vault-invoice-list loading ${className}`, children: [1, 2, 3].map((i) => (jsx("div", { className: "vault-invoice-skeleton" }, i))) }));
    }
    if (!invoices || invoices.length === 0) {
        return (jsx("div", { className: `vault-invoice-list empty ${className}`, style: {
                '--vault-foreground': appearance?.variables?.colorText,
            }, children: jsx("p", { className: "vault-invoice-empty-message", children: emptyMessage }) }));
    }
    return (jsxs("div", { className: `vault-invoice-list ${className}`, style: {
            '--vault-primary': appearance?.variables?.colorPrimary,
            '--vault-background': appearance?.variables?.colorBackground,
            '--vault-foreground': appearance?.variables?.colorText,
        }, children: [jsxs("table", { className: "vault-invoice-table", children: [jsx("thead", { children: jsxs("tr", { children: [jsx("th", { children: "Date" }), jsx("th", { children: "Amount" }), jsx("th", { children: "Status" }), jsx("th", { children: "Action" })] }) }), jsx("tbody", { children: invoices.map((invoice) => (jsxs("tr", { children: [jsx("td", { children: formatDate(invoice.createdAt) }), jsx("td", { children: formatAmount(invoice.total, invoice.currency) }), jsx("td", { children: jsx("span", { className: `vault-invoice-status ${getStatusColor(invoice.status)}`, children: invoice.status }) }), jsx("td", { children: invoice.invoicePdf && (jsx("a", { href: invoice.invoicePdf, target: "_blank", rel: "noopener noreferrer", className: "vault-invoice-download", children: "Download" })) })] }, invoice.id))) })] }), jsx("style", { children: `
        .vault-invoice-list {
          --vault-card-bg: var(--vault-background, #ffffff);
          --vault-text-primary: var(--vault-foreground, #111827);
          --vault-text-secondary: #6b7280;
