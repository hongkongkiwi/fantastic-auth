/**
 * Fantasticauth Tenant API Client
 */
...
export class TenantClient {
  ...
  private async request<T>(method: string, path: string, config: RequestConfig = {}): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new TenantError(message, code, response.status, details);
      }
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        throw new TenantError('Request timeout', 'TIMEOUT', 408);
      }

      throw new TenantError(error instanceof Error ? error.message : 'Unknown error', 'NETWORK_ERROR', 0);
    }
  }
  ...
  async *iterateUsers(query: Omit<ListUsersQuery, 'page' | 'perPage'> = {}): AsyncGenerator<TenantUserResponse, void, unknown> {
  ...
  async createUser(data: CreateUserRequest): Promise<TenantUserResponse> {
    return this.request('POST', '/admin/users', { body: data });
  }

  async getUser(userId: string): Promise<TenantUserResponse> {
    return this.request('GET', `/admin/users/${userId}`);
  }

  async updateUser(userId: string, data: UpdateUserRequest): Promise<TenantUserResponse> {
  ...
  async suspendUser(userId: string, data?: { reason?: string }): Promise<TenantUserResponse> {
    return this.request('POST', `/admin/users/${userId}/suspend`, { body: data ?? {} });
  }

  async activateUser(userId: string): Promise<TenantUserResponse> {
    return this.request('POST', `/admin/users/${userId}/activate`);
  }

  async listUserSessions(userId: string): Promise<TenantSessionListResponse> {
  ...
  ): AsyncGenerator<TenantOrganizationResponse, void, unknown> {
    yield* this.iteratePages(async (page, perPage) => {
  ...
  async getOrganization(orgId: string): Promise<TenantOrganizationResponse> {
    return this.request('GET', `/admin/organizations/${orgId}`);
  }

  async updateOrganization(orgId: string, data: UpdateOrgRequest): Promise<TenantOrganizationResponse> {
  ...
  async listOrganizationMembers(orgId: string): Promise<TenantOrganizationMemberResponse[]> {
