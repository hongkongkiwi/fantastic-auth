/**
 * Example: Tenant Dashboard with React
 * 
 * This example shows how to build a tenant dashboard
 * using the Fantasticauth Tenant SDK.
 */

import React, { useEffect, useState } from 'react';
import { TenantClient, UserManager } from '../src';

// Initialize client (in real app, get token from auth context)
const client = new TenantClient({
  baseUrl: process.env.VAULT_API_URL!,
  token: getTenantToken(), // Your auth function
  tenantId: 'my-tenant'
});
...
async function handleSuspend(userId: string) {
  if (confirm('Are you sure you want to suspend this user?')) {
    await users.suspend(userId, 'Suspended by tenant admin');
    // Refresh data
  }
}
